...
import {
  ...
  PARK_PERGOLA_X, PARK_PERGOLA_Z, PARK_PERGOLA_X1, PARK_PERGOLA_Z0, PARK_PERGOLA_Z1,
...
export function runMiamiParkWalkNSETests() {
  fails.length = 0;
  passedCount = 0;
  ...
  const westNS = gardenPathGeom(PARK_WALK_NS_X, PARK_WALK_NS_Z);
  const geom = gardenPathGeom(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z);
  ...
  const westNSSlabs = gardenPathSlabs(westNS);
  ...
  ok('walk walks 85.2→92.8 in z', PARK_WALK_NS_E_Z0 === 85.2 && PARK_WALK_NS_E_Z1 === 92.8);
  ok('walk centre is signed x=280 / z=89', PARK_WALK_NS_E_X === 280
    && PARK_WALK_NS_E_Z === 89);
  ok('walk width is 1.6 m (x 279.2–280.8)',
    PARK_WALK_NS_E_W === 1.6 && PARK_WALK_NS_E_X0 === 279.2 && PARK_WALK_NS_E_X1 === 280.8
    && Math.abs(PARK_WALK_NS_E_X1 - PARK_WALK_NS_E_X0 - PARK_WALK_NS_E_W) < 1e-9
    && PARK_WALK_NS_E_W === GARDEN_PATH_W
    && PARK_WALK_NS_E_W === PARK_WALK_W
    && PARK_WALK_NS_E_W === PARK_WALK_E_W
    && PARK_WALK_NS_E_W === PARK_WALK_NS_W);
  ok('walk length is 7.6 m', PARK_WALK_NS_E_LEN === 7.6
    && Math.abs(PARK_WALK_NS_E_Z1 - PARK_WALK_NS_E_Z0 - PARK_WALK_NS_E_LEN) < 1e-9
    && PARK_WALK_NS_E_LEN === PARK_WALK_NS_LEN);
  ok('geom matches signed constants',
    geom.x0 === 279.2 && geom.x1 === 280.8 && geom.x === 280
    && geom.z0 === 85.2 && geom.z1 === 92.8 && geom.z === 89
    && geom.w === 1.6 && geom.len === 7.6);
  ok('x/z were not invented or slid',
    geom.x === PARK_WALK_NS_E_X && geom.z === PARK_WALK_NS_E_Z
    && PARK_WALK_NS_E_X === 280 && PARK_WALK_NS_E_X0 === 279.2
    && PARK_WALK_NS_E_X1 === 280.8 && PARK_WALK_NS_E_Z0 === 85.2
    && PARK_WALK_NS_E_Z1 === 92.8);
  ok('280 is +4 m off 276 (mirror of 272)',
    Math.abs(PARK_WALK_NS_E_X - 276 - 4) < 1e-9
    && Math.abs(276 - PARK_WALK_NS_X - 4) < 1e-9
    && PARK_WALK_NS_X === 272 && PARK_WALK_NS_E_X === 280);

  ok('0.4 m off the 84 walk (z1=84.8)',
    Math.abs(PARK_WALK_NS_E_Z0 - GARDEN_PATH_Z1 - 0.4) < 1e-9
    && PARK_WALK_NS_E_Z0 === 85.2 && GARDEN_PATH_Z1 === 84.8);
  ok('0.4 m off the east walk (z0=93.2)',
    Math.abs(PARK_WALK_E_Z0 - PARK_WALK_NS_E_Z1 - 0.4) < 1e-9
    && PARK_WALK_NS_E_Z1 === 92.8 && PARK_WALK_E_Z0 === 93.2);
  ok('near-T, not a kiss — does not extend into either slab run',
    geom.z0 > GARDEN_PATH_Z1
    && geom.z1 < PARK_WALK_E_Z0
    && gardenSlabs.every((s) => s.z1 < geom.z0)
    && eastSlabs.every((s) => s.z0 > geom.z1)
    ...
    && !eastSlabs.some((s) => s.x1 > geom.x0 - 1e-9 && s.x0 < geom.x1 + 1e-9
      && s.z1 > geom.z0 - 1e-9 && s.z0 < geom.z1 + 1e-9));
  ok("T's the east walk in x (280 sits in 277.8→284), not 272",
    PARK_WALK_NS_E_X > PARK_WALK_E_X0 && PARK_WALK_NS_E_X < PARK_WALK_E_X1
    && PARK_WALK_E_X0 === 277.8 && PARK_WALK_E_X1 === 284
    && PARK_WALK_NS_E_X === 280
    && PARK_WALK_NS_E_X !== 272
    && !(PARK_WALK_NS_E_X > PARK_WALK_X0 && PARK_WALK_NS_E_X < PARK_WALK_X1));
  ok('misses 276/90 by ~2.3 m',
    Math.abs(PARK_WALK_NS_E_X0 - PARK_BENCH_X1 - 2.3) < 1e-9
    && PARK_BENCH_X === 276 && PARK_BENCH_Z === 90
    && PARK_WALK_NS_E_X0 === 279.2);
  ok('misses the pergola posts on x=276',
    PARK_WALK_NS_E_X0 > PARK_PERGOLA_X1
    && PARK_WALK_NS_E_X0 > (PARK_PERGOLA_X + GATE_HALF_X)
    && PARK_PERGOLA_X === 276
    && GATE_HALF_X === 1.15
    && Math.abs(PARK_PERGOLA_X1 - 277.15) < 1e-9);

  ok('walk is not pavement', !onPavement(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z));
  ok('walk is not boardwalk', !onBoardwalk(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z));
  ok('walk is not roadway', !onRoadway(PARK_WALK_NS_E_Z));
  ok('walk is not a cross-street', !onCrossStreet(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z));
  ok('walk is not a sidewalk slab', !onSidewalk(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z));
  ok('walk sits on leftover-city grade',
    groundHeight(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z) === CITY_Y);
  ok('walk is reserved', inReserved(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z));
  ok('walk is a keepout', inKeepout(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z));
  ok('reservedOverlap covers the signed walk',
    reservedOverlap(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z, PARK_WALK_NS_E_W, PARK_WALK_NS_E_LEN, 0.15));
  ok('tryPlace drops the reserved walk',
    tryPlace(ctx, PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z) === 0);
  ok('tryPlace does not remap the walk',
    tryPlace(ctx, PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z) === 0);
  ok('signed cell is not rejected', !gardenPathRejected(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z));
  ok('walk footprint is not in the street',
    !streetOverlap(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z, PARK_WALK_NS_E_W, PARK_WALK_NS_E_LEN));
  ok('inGardenPath covers the signed east N-S walk',
    inGardenPath(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z)
    && inGardenPath(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z0)
    && inGardenPath(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z1)
    && inGardenPath(PARK_WALK_NS_E_X0, PARK_WALK_NS_E_Z)
    && inGardenPath(PARK_WALK_NS_E_X1, PARK_WALK_NS_E_Z));

  // ---- west / east walks / west N-S / garden path / seats / lots stay put
  ...
  ok('west N-S stays x=272, 85.2→92.8',
    PARK_WALK_NS_X === 272 && PARK_WALK_NS_Z === 89
    && PARK_WALK_NS_X0 === 271.2 && PARK_WALK_NS_X1 === 272.8
    && PARK_WALK_NS_Z0 === 85.2 && PARK_WALK_NS_Z1 === 92.8
    && PARK_WALK_NS_W === 1.6 && PARK_WALK_NS_LEN === 7.6
    && westNS.x === 272 && westNS.x0 === 271.2 && westNS.x1 === 272.8
    && westNS.z0 === 85.2 && westNS.z1 === 92.8);
  ...
  ok('garden path was not slid onto the east N-S run',
    garden.z === 84 && garden.x1 === 284 && geom.x === 280 && geom.z0 === 85.2);
  ok('west walk was not slid onto 85.2→92.8',
    west.x0 === 268 && west.x1 === 274.2 && west.z === 94
    && geom.z0 === 85.2 && geom.z1 === 92.8);
  ok('east walk was not slid onto 279.2–280.8',
    east.x0 === 277.8 && east.x1 === 284 && east.z === 94
    && geom.x0 === 279.2 && geom.x1 === 280.8);
  ok('west N-S was not slid onto 279.2–280.8',
    westNS.x0 === 271.2 && westNS.x1 === 272.8 && westNS.x === 272
    && geom.x0 === 279.2 && geom.x1 === 280.8 && geom.x === 280);
  ...
  ok('does not overlap leftoverLot A/B/C/D reserved',
    !leftoverLotOverlap(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z, PARK_WALK_NS_E_W, PARK_WALK_NS_E_LEN, 0.15)
    && !inLeftoverLotReserved(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z)
    && !inLeftoverLotReserved(PARK_WALK_NS_E_X0, PARK_WALK_NS_E_Z)
    && !inLeftoverLotReserved(PARK_WALK_NS_E_X1, PARK_WALK_NS_E_Z)
    && !inLeftoverLotReserved(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z0)
    && !inLeftoverLotReserved(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z1));
  ok('does not sit on garden path 268→284 / z=84',
    geom.z0 > GARDEN_PATH_Z1
    && gardenSlabs.every((s) => s.z1 < geom.z0));
  ok('does not sit on west park walk 268→274.2 / z=94',
    geom.x0 > west.x1
    && westSlabs.every((s) => s.x1 < geom.x0)
    && !westSlabs.some((s) => s.x1 > geom.x0 - 1e-9 && s.x0 < geom.x1 + 1e-9
      && s.z1 > geom.z0 - 1e-9 && s.z0 < geom.z1 + 1e-9));
  ok('does not sit on east park walk 277.8→284 / z=94',
    geom.z1 < east.z0
    && eastSlabs.every((s) => s.z0 > geom.z1));
  ok('does not sit on west N-S 272 / 85.2→92.8',
    geom.x0 > westNS.x1
    && westNSSlabs.every((s) => s.x1 < geom.x0)
    && !westNSSlabs.some((s) => s.x1 > geom.x0 - 1e-9 && s.x0 < geom.x1 + 1e-9
      && s.z1 > geom.z0 - 1e-9 && s.z0 < geom.z1 + 1e-9));
  ...
  ok('tryPlace still drops the west N-S',
  ...
  ok('drop if the kit kisses z=94 slabs',
    gardenPathRejected(PARK_WALK_E_X, PARK_WALK_E_Z - 1) === true);
  ...
  // ---- shared slab kit; no gardenPathBGeom / parkWalkNSEGeom fork --------
  ...
  ok('east N-S reuses gardenPathGeom, same slab kit',
  ...
  ok('slabs stay inside the signed walk',
    slabs.every((s) => s.x0 >= PARK_WALK_NS_E_X0 - 1e-9
      && s.x1 <= PARK_WALK_NS_E_X1 + 1e-9
      && s.z0 >= PARK_WALK_NS_E_Z0 - 1e-9
      && s.z1 <= PARK_WALK_NS_E_Z1 + 1e-9));
  ...
  ok('slabs do not enter the sash',
    slabs.every((s) => s.x0 > sash.x1 && s.x0 > PARK_PERGOLA_X1));
  ...
  ok('slabs do not enter the 84 walk or z=94 walks',
    slabs.every((s) => s.z0 > GARDEN_PATH_Z1 && s.z1 < PARK_WALK_E_Z0)
    && slabs.every((s) => s.z1 < east.z0 && s.x0 > west.x1));
  ...
  ok('no filled path AABB',
    !aabbs.some((s) => s.sz >= PARK_WALK_NS_E_LEN - 0.4
      && s.sx >= PARK_WALK_NS_E_W - 0.4 && s.sy >= 0.4));
  ok('east N-S park walk AABB flag is false',
    PARK_WALK_NS_E_AABB === false && GARDEN_PATH_AABB === false);
  ...
  ok('walk air void does not reach the sash',
    air.x0 > sash.x1 && air.x0 === PARK_WALK_NS_E_X0
    && air.z1 < sash.z0);

  // ---- one grass hull at grade; not per-blade colliders (Sylva) ---------
  ok('one grass hull covers the signed walk',
    hull.x0 === PARK_WALK_NS_E_X0 && hull.x1 === PARK_WALK_NS_E_X1
    && hull.z0 === PARK_WALK_NS_E_Z0 && hull.z1 === PARK_WALK_NS_E_Z1
    && hull.y0 === CITY_Y);
  ...
  ok('grass hull area is width × length',
    Math.abs(hullArea(hull) - PARK_WALK_NS_E_LEN * PARK_WALK_NS_E_W) < 1e-9);
  ok('no grass-hull AABB in the collider bag',
    !aabbs.some((s) => Math.abs(s.sz - PARK_WALK_NS_E_LEN) < 0.2
      && Math.abs(s.sx - PARK_WALK_NS_E_W) < 0.2));
  ...
  const nsTest = readFileSync(join(here, 'parkWalkNSTest.js'), 'utf8');
  ...
  ok('east N-S walk reuses gardenPathGeom, no parkWalkNSEGeom fork',
    gardenSrc.includes('gardenPathGeom(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z)')
    && gardenSrc.includes('gardenPathRejected(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z)')
    && gardenSrc.includes('onPavement(PARK_WALK_NS_E_X, PARK_WALK_NS_E_Z)')
    && constants.includes('export function gardenPathGeom')
    && constants.includes('280 / 85.2→92.8')
  ...
  ok('did not ship parkWalkNSE.js / gardenPathNSE.js',
    ...
    && !existsSync(join(here, 'landmarks/parkWalkNSE.js'))
    ...
    && !existsSync(join(here, 'landmarks/gardenPathNSE.js'))
    && !existsSync(join(here, 'parkWalkNSE.js')));
  ...
  ok('kit is Tiny Glade flagstones + one grass hull',
    ...
    && gardenSrc.includes('280') && gardenSrc.includes('Desi')
  ...
  ok('west park walk 268→274.2 / z=94 was not restacked',
    ...
    && !westTest.includes('PARK_WALK_NS_E_') && !westTest.includes('279.2')
  ...
  ok('east park walk 277.8→284 / z=94 was not restacked',
    ...
    && !eastTest.includes('PARK_WALK_NS_E_') && !eastTest.includes('279.2')
    && constants.includes('PARK_WALK_E_X0 = 277.8')
    && constants.includes('PARK_WALK_E_X1 = 284')
    && constants.includes('PARK_WALK_E_Z = 94'));
  ok('west N-S 272 / 85.2→92.8 was not restacked',
    gardenSrc.includes('272') && gardenSrc.includes('85.2')
    && nsTest.includes('PARK_WALK_NS_X === 272') && nsTest.includes('85.2→92.8')
    && !nsTest.includes('PARK_WALK_NS_E_') && !nsTest.includes('279.2')
    && constants.includes('PARK_WALK_NS_X = 272')
    && constants.includes('PARK_WALK_NS_X0 = 271.2')
    && constants.includes('PARK_WALK_NS_X1 = 272.8'));
  ...
  if (fails.length) {
    console.error('[miami-parkWalkNSE] FAIL');
    for (const f of fails) console.error('  -', f);
  } else {
    console.log('[miami-parkWalkNSE] ok', passedCount, 'checks');
  }
  return { passed: fails.length === 0, fails, passedCount };
}

const isMain = typeof process !== 'undefined'
  && process.argv[1] && process.argv[1].endsWith('parkWalkNSETest.js');
if (isMain) {
  const r = runMiamiParkWalkNSETests();
