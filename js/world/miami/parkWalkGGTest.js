// Headless checks for the Miami G-park spine (garden-path slab kit).
...
export function runMiamiParkWalkGGTests() {
  fails.length = 0;
  passedCount = 0;
  ...
  const geom = gardenPathGeom(PARK_WALK_GG_X, PARK_WALK_GG_Z);
  ...
  ok('walk walks 373→389 in x', PARK_WALK_GG_X0 === 373 && PARK_WALK_GG_X1 === 389);
  ok('walk centre is signed z=96', PARK_WALK_GG_Z === 96
    && PARK_WALK_GG_X === 381);
  ok('walk width is 1.6 m (z 95.2–96.8)',
    PARK_WALK_GG_W === 1.6 && PARK_WALK_GG_Z0 === 95.2 && PARK_WALK_GG_Z1 === 96.8
    && Math.abs(PARK_WALK_GG_Z1 - PARK_WALK_GG_Z0 - PARK_WALK_GG_W) < 1e-9
    && PARK_WALK_GG_W === GARDEN_PATH_W
    && PARK_WALK_GG_W === PARK_WALK_W
    && PARK_WALK_GG_W === PARK_WALK_FF_W);
  ok('walk length is 16 m (16 × 1.6 on the 381/96 hull)', PARK_WALK_GG_LEN === 16
    && Math.abs(PARK_WALK_GG_X1 - PARK_WALK_GG_X0 - PARK_WALK_GG_LEN) < 1e-9
    && PARK_WALK_GG_LEN === GARDEN_PATH_LEN
    && PARK_WALK_GG_LEN === PARK_WALK_FF_LEN);
  ok('6 m inland of leftoverLot G (G z1=90)',
    LEFTOVER_LOT_G_Z1 === 90 && PARK_WALK_GG_Z === 96
    && Math.abs(PARK_WALK_GG_Z - LEFTOVER_LOT_G_Z1 - 6) < 1e-9);
  ok('geom matches signed constants',
    geom.x0 === 373 && geom.x1 === 389 && geom.z === 96
    && geom.z0 === 95.2 && geom.z1 === 96.8 && geom.w === 1.6
    && geom.x === 381 && geom.len === 16);
  ok('x/z were not invented or slid',
    geom.x === PARK_WALK_GG_X && geom.z === PARK_WALK_GG_Z
    && PARK_WALK_GG_X0 === 373 && PARK_WALK_GG_X1 === 389 && PARK_WALK_GG_Z === 96);
  ok('last slab stays inside 389',
    PARK_WALK_GG_X1 === 389
    && slabs.every((s) => s.x1 <= 389 + 1e-9)
    && lastSlab && lastSlab.x1 <= 389 + 1e-9);
  ok('did not grow past 389',
    geom.x1 === 389 && PARK_WALK_GG_X1 === 389
    && slabs.every((s) => s.x1 <= PARK_WALK_GG_X1 + 1e-9));
  ok('sits on the signed 381/96 hull',
    geom.x0 === POCKET_PARK_G_X0 && geom.x1 === POCKET_PARK_G_X1
    && geom.z >= POCKET_PARK_G_Z0 && geom.z <= POCKET_PARK_G_Z1
    && parkG.x === 381 && parkG.z === 96
    && parkG.x0 === 373 && parkG.x1 === 389);
  ok('does not merge with PARK_WALK_FF_X1=372 (1 m west gap, same z)',
    PARK_WALK_FF_X1 === 372 && PARK_WALK_GG_X0 === 373
    && PARK_WALK_GG_X0 === PARK_WALK_FF_X1 + 1
    && PARK_WALK_FF_Z === PARK_WALK_GG_Z
    && PARK_WALK_FF_Z0 === PARK_WALK_GG_Z0
    && PARK_WALK_FF_Z1 === PARK_WALK_GG_Z1
    && geom.x0 > ff.x1
    && !ffSlabs.some((s) => s.x1 > geom.x0 - 1e-9 && s.x0 < geom.x1 + 1e-9
      && s.z1 > geom.z0 - 1e-9 && s.z0 < geom.z1 + 1e-9));

  ok('walk is not pavement', !onPavement(PARK_WALK_GG_X, PARK_WALK_GG_Z));
  ok('walk is not boardwalk', !onBoardwalk(PARK_WALK_GG_X, PARK_WALK_GG_Z));
  ok('walk is not roadway', !onRoadway(PARK_WALK_GG_Z));
  ok('walk is not a cross-street', !onCrossStreet(PARK_WALK_GG_X, PARK_WALK_GG_Z));
  ok('walk is not a sidewalk slab', !onSidewalk(PARK_WALK_GG_X, PARK_WALK_GG_Z));
  ok('walk sits on leftover-city grade',
    groundHeight(PARK_WALK_GG_X, PARK_WALK_GG_Z) === CITY_Y);
  ok('walk is reserved', inReserved(PARK_WALK_GG_X, PARK_WALK_GG_Z));
  ok('walk is a keepout', inKeepout(PARK_WALK_GG_X, PARK_WALK_GG_Z));
  ok('reservedOverlap covers the signed walk',
    reservedOverlap(PARK_WALK_GG_X, PARK_WALK_GG_Z, PARK_WALK_GG_LEN, PARK_WALK_GG_W, 0.15));
  ok('tryPlace drops the reserved walk',
    tryPlace(ctx, PARK_WALK_GG_X, PARK_WALK_GG_Z) === 0);
  ok('tryPlace does not remap the walk',
    tryPlace(ctx, PARK_WALK_GG_X, PARK_WALK_GG_Z) === 0);
  ok('signed cell is not rejected', !gardenPathRejected(PARK_WALK_GG_X, PARK_WALK_GG_Z));
  ok('gardenPathRejected(381, 96) stays false after the wire-up',
    !gardenPathRejected(381, 96)
    && PARK_WALK_GG_X === 381 && PARK_WALK_GG_Z === 96);
  ok('walk footprint is not in the street',
    !streetOverlap(PARK_WALK_GG_X, PARK_WALK_GG_Z, PARK_WALK_GG_LEN, PARK_WALK_GG_W));
  ok('inGardenPath covers the signed G-park spine',
    inGardenPath(PARK_WALK_GG_X, PARK_WALK_GG_Z)
    && inGardenPath(PARK_WALK_GG_X0, PARK_WALK_GG_Z)
    && inGardenPath(PARK_WALK_GG_X1, PARK_WALK_GG_Z));

  // ---- 276 park / walks / A–G / benches / pergola / EE / FF stay put ----
  ...
  ok('garden path was not slid onto z=96',
    garden.z === 84 && garden.x1 === 284 && geom.z === 96 && geom.x0 === 373);
  ok('EE spine was not slid onto 373→389',
    ee.x0 === 339 && ee.x1 === 355 && ee.z === 96
    && geom.x0 === 373 && geom.x1 === 389 && geom.z === 96);
  ...
  ok('FF spine was not slid onto 373→389',
    ff.x0 === 356 && ff.x1 === 372 && ff.z === 96
    && geom.x0 === 373 && geom.x1 === 389 && geom.z === 96);
  ...
  ok('leftoverLot A/B/C/D/E/F/G geometry was not slid',
  ...
  ok('leftoverLot G reserved z1+1.4=91.4 vs walk z0=95.2',
    LEFTOVER_LOT_G_Z1 + 1.4 === 91.4 && PARK_WALK_GG_Z0 === 95.2
    && PARK_WALK_GG_Z0 > LEFTOVER_LOT_G_Z1 + 1.4);
  ok('does not overlap leftoverLot A–G reserved',
    !leftoverLotOverlap(PARK_WALK_GG_X, PARK_WALK_GG_Z, PARK_WALK_GG_LEN, PARK_WALK_GG_W, 0.15)
    && leftoverLotOverlap(PARK_WALK_GG_X, PARK_WALK_GG_Z, PARK_WALK_GG_LEN, PARK_WALK_GG_W, 0.15) === false
    && !inLeftoverLotReserved(PARK_WALK_GG_X, PARK_WALK_GG_Z)
    && !inLeftoverLotReserved(PARK_WALK_GG_X0, PARK_WALK_GG_Z)
    && !inLeftoverLotReserved(PARK_WALK_GG_X1, PARK_WALK_GG_Z)
    && !inLeftoverLotReserved(PARK_WALK_GG_X, PARK_WALK_GG_Z0));
  ok('does not overlap warehouse reserved',
    !warehouseOverlap(PARK_WALK_GG_X, PARK_WALK_GG_Z, PARK_WALK_GG_LEN, PARK_WALK_GG_W, 0.15)
    && !inWarehouseReserved(PARK_WALK_GG_X, PARK_WALK_GG_Z));
  ok('does not sit on helipad E',
    !inHelipadReserved(PARK_WALK_GG_X, PARK_WALK_GG_Z)
    && PARK_WALK_GG_X1 < 408);
  ok('does not sit on garden path 268→284 / z=84',
    ...
    && !gardenSlabs.some((s) => s.x1 > geom.x0 - 1e-9 && s.x0 < geom.x1 + 1e-9
      && s.z1 > geom.z0 - 1e-9 && s.z0 < geom.z1 + 1e-9));
  ok('does not sit on 276 park hull (x1=284 < 373)',
    geom.x0 > POCKET_PARK_X1
    && POCKET_PARK_X1 === 284 && PARK_WALK_GG_X0 === 373
  ...
  ok('tryPlace still drops leftoverLot A–G',
  ...
  // ---- drop if it kisses leftoverLot G / helipad / warehouse / 276 ------
  ...
  ok('drop if the kit kisses FF spine slabs (F-park hull merge)',
    gardenPathRejected(PARK_WALK_FF_X, PARK_WALK_FF_Z + 1) === true);
  ok('drop pavement / streetOverlap, never nudge',
    gardenPathRejected(0, 27) === true && gardenPathRejected(57, 80) === true);

  // ---- shared slab kit; no parkWalkGGGeom / gardenPathGGeom fork ---------
  ...
  ok('G-park spine reuses gardenPathGeom, same slab kit',
  ...
  ok('slabs stay inside the signed walk',
    slabs.every((s) => s.x0 >= PARK_WALK_GG_X0 - 1e-9
      && s.x1 <= PARK_WALK_GG_X1 + 1e-9
      && s.z0 >= PARK_WALK_GG_Z0 - 1e-9
      && s.z1 <= PARK_WALK_GG_Z1 + 1e-9));
  ...
  ok('no filled path AABB',
    !aabbs.some((s) => s.sx >= PARK_WALK_GG_LEN - 0.4
      && s.sz >= PARK_WALK_GG_W - 0.4 && s.sy >= 0.4));
  ok('G-park spine AABB flag is false',
    PARK_WALK_GG_AABB === false && GARDEN_PATH_AABB === false);
  ...
  ok('one grass hull covers the signed walk',
    hull.x0 === PARK_WALK_GG_X0 && hull.x1 === PARK_WALK_GG_X1
    && hull.z0 === PARK_WALK_GG_Z0 && hull.z1 === PARK_WALK_GG_Z1
    && hull.y0 === CITY_Y);
  ...
  ok('grass hull area is width × length',
    Math.abs(hullArea(hull) - PARK_WALK_GG_LEN * PARK_WALK_GG_W) < 1e-9);
  ok('walk eats ~26 m²',
    Math.abs(PARK_WALK_GG_LEN * PARK_WALK_GG_W - 25.6) < 1e-9);
  ok('no grass-hull AABB in the collider bag',
    !aabbs.some((s) => Math.abs(s.sx - PARK_WALK_GG_LEN) < 0.2
      && Math.abs(s.sz - PARK_WALK_GG_W) < 0.2));
  ...
  // ---- G leftover < 12800 after the walk; do not backfill ---------------
  ok('G park hull is not rejected by the spine',
  ...
  ok('G planned grid stays empty-hull 12800 (cover not raised)',
  ...
  ok('did not backfill slabs to hit empty-hull 12800',
    ...
    && fieldG.cells.some((c) => inGardenPath(c.x, c.z)));
  ok('no G blade on the spine / lot G / helipad / warehouse',
    fieldG.placed.every((p) => !pocketParkDrop(p.x, p.z)
      && !inGardenPath(p.x, p.z)
      && !inLeftoverLotReserved(p.x, p.z)
      && !inWarehouseReserved(p.x, p.z)
      && !inHelipadReserved(p.x, p.z)));
  ok('F leftover stays 8000–11000 after the G spine',
  ...
  ok('E leftover stays 8000–11000 after the G spine',
  ...
  ok('G-park spine reuses gardenPathGeom, no parkWalkGGGeom fork',
    gardenSrc.includes('gardenPathGeom(PARK_WALK_GG_X, PARK_WALK_GG_Z)')
    && gardenSrc.includes('gardenPathRejected(PARK_WALK_GG_X, PARK_WALK_GG_Z)')
    && gardenSrc.includes('onPavement(PARK_WALK_GG_X, PARK_WALK_GG_Z)')
  ...
  ok('did not ship parkWalkGG.js / gardenPathG.js',
    ...
    && !existsSync(join(here, 'parkWalkGG.js')));
  ...
  ok('kit is Tiny Glade flagstones + one grass hull',
    ...
    && gardenSrc.includes('373') && gardenSrc.includes('Desi')
  ...
  ok('276 walks were not restacked',
    ...
    && !westTest.includes('PARK_WALK_GG_') && !eastTest.includes('PARK_WALK_GG_')
  ...
  ok('leftoverLot A–G were not restacked',
  ...
  if (fails.length) {
    console.error('[miami-parkWalkGG] FAIL');
    for (const f of fails) console.error('  -', f);
  } else {
    console.log('[miami-parkWalkGG] ok', passedCount, 'checks',
      `lastSlab.x1=${lastSlab.x1} placedG=${fieldG.placed.length}/${plannedG} placedF=${fieldF.placed.length} placedE=${fieldE.placed.length} placed276=${field276.placed.length}`);
  }
  return {
...
const isMain = typeof process !== 'undefined'
  && process.argv[1] && process.argv[1].endsWith('parkWalkGGTest.js');
if (isMain) {
  const r = runMiamiParkWalkGGTests();
