...
import {
  CITY_Y, GAP501_SOFFIT, GAP501_H, GAP501_PASS_H, GAP501_PASS_W, GAP501_D,
  gap501Shops, installFlyColliders,
} from '../constants.js';
import { cBox, cCyl } from '../geo.js';
import { stuccoTexture } from '../textures.js';

// ============================================================
// GAP_X=-501 inland storefronts with arcade fly-unders (±Z) and a
...
const PALETTE = [
  ...
  { body: 0xf6f2e9, trim: 0x1d6f7a, neon: 0x2fe0ff },
  { body: 0xd4e8f4, trim: 0xe89aab, neon: 0xff4fa3 },
  { body: 0xf4d6c0, trim: 0x7fd4c1, neon: 0xffc43a },
  { body: 0xc5e4d8, trim: 0x2f6f86, neon: 0x2fe0ff },
  { body: 0xf3d56a, trim: 0xe89aab, neon: 0xff4fa3 },
  { body: 0xe9f0e6, trim: 0x1d6f7a, neon: 0xffc43a },
  { body: 0xe89aab, trim: 0xf6f2e9, neon: 0xff4fa3 },
  { body: 0xf6e2b3, trim: 0x2f6f86, neon: 0x2fe0ff },
];

/**
 * Build signed GAP_X=-501 storefronts inland of Ocean Drive.
 * @returns {{ group: THREE.Group }}
 */
export function buildGap501(ctx) {
  ...
  for (let i = 0; i < shops.length; i++) {
    buildGap501Shop(stucco, dark, glass, pushNeon, addCollider, shops[i], PALETTE[i % PALETTE.length]);
  }
  installFlyColliders(addCyl, addCollider, 'gap501');
...
function buildGap501Shop(stucco, dark, glass, pushNeon, addCollider, g, pal) {
  const { body, trim, neon } = pal;
  const cx = g.x;
  const cz = g.z;
  const len = g.len;
  const D = GAP501_D;
  const bodyH = GAP501_H;
  const soffit = GAP501_SOFFIT;
  const face = g.frontX;
  ...
  stucco.push(cBox(massSx, 0.22, GAP501_PASS_W + 0.55, trim,
    massX, CITY_Y + GAP501_PASS_H + 0.11, cz));
