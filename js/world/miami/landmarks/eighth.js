...
import {
  CITY_Y, EIGHTH_SOFFIT, EIGHTH_H, EIGHTH_PASS_H, EIGHTH_PASS_W, EIGHTH_D,
  eighthShops, installFlyColliders,
} from '../constants.js';
import { cBox, cCyl } from '../geo.js';
import { stuccoTexture } from '../textures.js';

// ============================================================
// 8th-street analogue (GAP_X=-129) — inland storefronts with arcade
// fly-unders (±Z) and a mid-block passage (±X). Colliders are
// jambs + soffit + passage walls; never a box in a bay. hash01
// never drawn. West of leftoverLot A. Not a travel-lane solid.
// New RESERVED west of x=240.
// ============================================================

const PALETTE = [
  { body: 0xe8c4a8, trim: 0x2f6f86, neon: 0xffb01f },
  { body: 0xf6f2e9, trim: 0xe89aab, neon: 0xff4fa3 },
  { body: 0xc5e4d8, trim: 0xf6f2e9, neon: 0x2fe0ff },
  { body: 0xf3d56a, trim: 0x1d6f7a, neon: 0xffc43a },
  { body: 0xd4e8f4, trim: 0xf6f2e9, neon: 0x2fe0ff },
  { body: 0xe89aab, trim: 0x7fd4c1, neon: 0xff4fa3 },
];

/**
 * Build signed 8th-street storefronts inland of Ocean Drive.
 * @returns {{ group: THREE.Group }}
 */
export function buildEighth(ctx) {
  ...
  for (let i = 0; i < shops.length; i++) {
    buildEighthShop(stucco, dark, glass, pushNeon, addCollider, shops[i], PALETTE[i % PALETTE.length]);
  }
  installFlyColliders(addCyl, addCollider, 'eighth');

  const group = new THREE.Group();
  group.name = 'eighth-street';
...
function buildEighthShop(stucco, dark, glass, pushNeon, addCollider, g, pal) {
  const { body, trim, neon } = pal;
  const cx = g.x;
  const cz = g.z;
  const len = g.len;
  const D = EIGHTH_D;
  const bodyH = EIGHTH_H;
  const soffit = EIGHTH_SOFFIT;
  const face = g.frontX;
  ...
  stucco.push(cBox(massSx, 0.22, EIGHTH_PASS_W + 0.55, trim,
    massX, CITY_Y + EIGHTH_PASS_H + 0.11, cz));
