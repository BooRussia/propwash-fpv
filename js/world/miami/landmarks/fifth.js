...
// ============================================================
// 5th-street analogue (GAP_X=57) — inland storefronts with arcade
// fly-unders (±Z) and a mid-block passage (±X). Colliders are
// jambs + soffit + passage walls; never a box in a bay. hash01
// never drawn. Not leftoverLot. Not a travel-lane solid.
// ============================================================

const PALETTE = [
  { body: 0xf4b8c5, trim: 0xf6f2e9, neon: 0xff4fa3 },
  { body: 0xf6f2e9, trim: 0x7fd4c1, neon: 0x2fe0ff },
  { body: 0xd8ece8, trim: 0xf6f2e9, neon: 0x2fe0ff },
  { body: 0xf7dcb4, trim: 0xf6f2e9, neon: 0xffb01f },
  { body: 0xf3d56a, trim: 0x2f6f86, neon: 0xffc43a },
  { body: 0xe89aab, trim: 0xf6f2e9, neon: 0xff4fa3 },
  { body: 0xd4e8f4, trim: 0x1d6f7a, neon: 0x2fe0ff },
];

/**
 * Build signed 5th-street storefronts inland of Ocean Drive.
 * @returns {{ group: THREE.Group }}
 */
export function buildFifth(ctx) {
  ...
  for (let i = 0; i < shops.length; i++) {
    buildFifthShop(stucco, dark, glass, pushNeon, addCollider, shops[i], PALETTE[i % PALETTE.length]);
  }
  installFlyColliders(addCyl, addCollider, 'fifth');

  const group = new THREE.Group();
  group.name = 'fifth-street';
...
function buildFifthShop(stucco, dark, glass, pushNeon, addCollider, g, pal) {
