...
import {
  CITY_Y, ESPA_SOFFIT, ESPA_H, ESPA_PASS_H, ESPA_PASS_W, ESPA_D,
  espaShops, installFlyColliders,
} from '../constants.js';
import { cBox, cCyl } from '../geo.js';
import { stuccoTexture } from '../textures.js';

// ============================================================
// Espanola Way analogue (GAP_X=243) — west-face storefronts only.
// East face would restack leftoverLot A (x0=251). Arcade fly ±Z,
// mid-block passage fly ±X. Jambs + soffit only. hash01 never
// drawn. New RESERVED west of x=240. Not a travel-lane solid.
// ============================================================
...
/**
 * Build signed Espanola storefronts east of the cinema, west of leftoverLot A.
 * @returns {{ group: THREE.Group }}
 */
export function buildEspa(ctx) {
  ...
  const shops = espaShops();
  for (let i = 0; i < shops.length; i++) {
    buildEspaShop(stucco, dark, glass, pushNeon, addCollider, shops[i], PALETTE[i % PALETTE.length]);
  }
  installFlyColliders(addCyl, addCollider, 'espa');

  const group = new THREE.Group();
  group.name = 'espanola-way';
...
function buildEspaShop(stucco, dark, glass, pushNeon, addCollider, g, pal) {
  ...
  stucco.push(cBox(massSx, 0.22, ESPA_PASS_W + 0.55, trim,
    massX, CITY_Y + ESPA_PASS_H + 0.11, cz));
