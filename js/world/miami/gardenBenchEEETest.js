// Headless checks for the Miami E-park east bench (Tiny Glade 3-seat slat).
...
import {
  ...
  PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z, PARK_BENCH_EE_E_YAW, PARK_BENCH_EE_E_W,
  PARK_BENCH_EE_E_DEPTH, PARK_BENCH_EE_E_SEAT_H, PARK_BENCH_EE_E_BACK_H,
  PARK_BENCH_EE_E_UNDER_CLEAR,
  PARK_BENCH_EE_E_X0, PARK_BENCH_EE_E_X1, PARK_BENCH_EE_E_Z0, PARK_BENCH_EE_E_Z1,
...
export function runMiamiGardenBenchEEETests() {
  fails.length = 0;
  passedCount = 0;

  const ctx = { blocked: () => false };
  const geom = gardenBenchGeom(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z);
  const parts = gardenBenchParts(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z);
  ...
  const geomEEW = gardenBenchGeom(PARK_BENCH_EE_W_X, PARK_BENCH_EE_W_Z);
  ...
  ok('E-park east bench cell is signed 353.5 / 94.4',
    PARK_BENCH_EE_E_X === 353.5 && PARK_BENCH_EE_E_Z === 94.4);
  ok('+6.5 m off 347 (347 + 6.5 = 353.5)',
    Math.abs(PARK_BENCH_EE_E_X - PARK_BENCH_EE_X - 6.5) < 1e-9
    && PARK_BENCH_EE_X === 347);
  ok('353.5 is the mirror of 340.5 (347 − 6.5)',
    Math.abs(PARK_BENCH_EE_X - PARK_BENCH_EE_W_X - 6.5) < 1e-9
    && Math.abs(PARK_BENCH_EE_E_X + PARK_BENCH_EE_W_X - 2 * PARK_BENCH_EE_X) < 1e-9
    && PARK_BENCH_EE_W_X === 340.5);
  ok('E-park east bench x/z were not invented or slid',
    geom.x === 353.5 && geom.z === 94.4
    && geom.x === PARK_BENCH_EE_E_X && geom.z === PARK_BENCH_EE_E_Z
  ...
  ok('E-park east bench yaw faces +Z toward the spine at z=96',
    PARK_BENCH_EE_E_YAW === 0 && PARK_BENCH_EE_E_YAW === PARK_BENCH_EE_YAW
    && geom.yaw === PARK_BENCH_EE_E_YAW
    && geom.yaw === 0 && geom.yaw !== PARK_BENCH_YAW
    && geomEE.yaw === 0 && geomEEW.yaw === 0);
  ok('E-park east bench is the same 1.8 m 3-seat kit',
    PARK_BENCH_EE_E_W === 1.8 && PARK_BENCH_EE_E_W === GARDEN_BENCH_W
    && PARK_BENCH_EE_E_W === PARK_BENCH_W && geom.w === GARDEN_BENCH_W
    && PARK_BENCH_EE_W === 1.8 && PARK_BENCH_EE_W_W === 1.8);
  ok('E-park east bench seat H is 0.43–0.46 m',
    PARK_BENCH_EE_E_SEAT_H >= 0.43 && PARK_BENCH_EE_E_SEAT_H <= 0.46
    && PARK_BENCH_EE_E_SEAT_H === GARDEN_BENCH_SEAT_H
    && geom.seatH === GARDEN_BENCH_SEAT_H);
  ok('E-park east bench depth is ~0.45 m',
    Math.abs(PARK_BENCH_EE_E_DEPTH - 0.45) < 1e-9
    && PARK_BENCH_EE_E_DEPTH === GARDEN_BENCH_DEPTH
    && geom.depth === GARDEN_BENCH_DEPTH);
  ok('E-park east bench back crown is 0.80–0.90 m',
    PARK_BENCH_EE_E_BACK_H >= 0.80 && PARK_BENCH_EE_E_BACK_H <= 0.90
    && PARK_BENCH_EE_E_BACK_H === GARDEN_BENCH_BACK_H
    && geom.backH === GARDEN_BENCH_BACK_H);
  ok('E-park east bench under-slat clear is ~0.40 m',
    Math.abs(PARK_BENCH_EE_E_UNDER_CLEAR - 0.40) < 1e-9
    && PARK_BENCH_EE_E_UNDER_CLEAR === GARDEN_BENCH_UNDER_CLEAR
    && geom.underClear === GARDEN_BENCH_UNDER_CLEAR);
  ok('E-park east bench geom matches signed constants',
    geom.x === 353.5 && geom.z === 94.4 && geom.w === 1.8
    && Math.abs(geom.x0 - PARK_BENCH_EE_E_X0) < 1e-9
    && Math.abs(geom.x1 - PARK_BENCH_EE_E_X1) < 1e-9
    && Math.abs(geom.z0 - PARK_BENCH_EE_E_Z0) < 1e-9
    && Math.abs(geom.z1 - PARK_BENCH_EE_E_Z1) < 1e-9
    && PARK_BENCH_EE_E_X0 === 352.6 && PARK_BENCH_EE_E_X1 === 354.4);
  ok('0.8 m is 82.4 center-to-spine, not 269.5 edge-to-walk',
    PARK_WALK_EE_Z0 === 95.2
    && Math.abs(PARK_WALK_EE_Z0 - PARK_BENCH_EE_E_Z - 0.8) < 1e-9
    && Math.abs(PARK_WALK_EE_Z0 - PARK_BENCH_EE_E_Z1) !== 0.8
    && GARDEN_PATH_Z0 === 83.2
    && Math.abs(GARDEN_PATH_Z0 - GARDEN_BENCH_Z - 0.8) < 1e-9);
  ok('bench plate is ~0.8 m²',
    Math.abs(PARK_BENCH_EE_E_W * PARK_BENCH_EE_E_DEPTH - 0.81) < 1e-9);
  ok('sits on the E park hull (339–355 × 92–100)',
    PARK_BENCH_EE_E_X0 > POCKET_PARK_E_X0 && PARK_BENCH_EE_E_X1 < POCKET_PARK_E_X1
    && PARK_BENCH_EE_E_Z0 > POCKET_PARK_E_Z0 && PARK_BENCH_EE_E_Z1 < POCKET_PARK_E_Z1
    && POCKET_PARK_E_X0 === 339 && POCKET_PARK_E_X1 === 355
    && POCKET_PARK_E_Z0 === 92 && POCKET_PARK_E_Z1 === 100
    && POCKET_PARK_E_X === 347 && POCKET_PARK_E_Z === 96);
  ok('east end 354.4 stays inside hull x1=355',
    PARK_BENCH_EE_E_X1 === 354.4 && PARK_BENCH_EE_E_X1 < 355
    && POCKET_PARK_E_X1 === 355);
  ok('west end 352.6 stays east of 347/94.4 (x1=347.9)',
    PARK_BENCH_EE_E_X0 === 352.6 && PARK_BENCH_EE_X1 === 347.9
    && PARK_BENCH_EE_E_X0 > PARK_BENCH_EE_X1
    && Math.abs(PARK_BENCH_EE_E_X0 - PARK_BENCH_EE_X1 - 4.7) < 1e-9);
  ok('misses leftoverLot E (E z1=90)',
    LEFTOVER_LOT_E_Z1 === 90 && PARK_BENCH_EE_E_Z0 > LEFTOVER_LOT_E_Z1
    && !leftoverLotOverlap(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z,
      PARK_BENCH_EE_E_W, PARK_BENCH_EE_E_DEPTH, 0.15)
    && !inLeftoverLotReserved(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z)
    && !inLeftoverLotReserved(PARK_BENCH_EE_E_X0, PARK_BENCH_EE_E_Z)
    && !inLeftoverLotReserved(PARK_BENCH_EE_E_X1, PARK_BENCH_EE_E_Z));
  ...
  ok('340.5/94.4 stays',
    PARK_BENCH_EE_W_X === 340.5 && PARK_BENCH_EE_W_Z === 94.4
    && geomEEW.x === 340.5 && geomEEW.z === 94.4
    && PARK_BENCH_EE_W_X0 === 339.6 && PARK_BENCH_EE_W_X1 === 341.4
    && PARK_BENCH_EE_W_YAW === 0);
  ok('347/98.5 pergola stays',
    PARK_PERGOLA_EE_X === 347 && PARK_PERGOLA_EE_Z === 98.5);
  ok('existing benches stay 276/82.4, 276/90, 269.5/90, 282.5/90, 347/94.4, 340.5/94.4',
    GARDEN_BENCH_X === 276 && GARDEN_BENCH_Z === 82.4
    && PARK_BENCH_X === 276 && PARK_BENCH_Z === 90
    && PARK_BENCH_W_X === 269.5 && PARK_BENCH_W_Z === 90
    && PARK_BENCH_E_X === 282.5 && PARK_BENCH_E_Z === 90
    ...
    && gardenBenchGeom(PARK_BENCH_EE_W_X, PARK_BENCH_EE_W_Z).x === 340.5
    && gardenBenchGeom(PARK_BENCH_EE_W_X, PARK_BENCH_EE_W_Z).z === 94.4);
  ...
  ok('E-park east bench is not pavement',
    !onPavement(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z));
  ok('E-park east bench is not boardwalk',
    !onBoardwalk(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z));
  ok('E-park east bench is not roadway', !onRoadway(PARK_BENCH_EE_E_Z));
  ok('E-park east bench is not a cross-street',
    !onCrossStreet(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z));
  ok('E-park east bench is not a sidewalk slab',
    !onSidewalk(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z));
  ok('E-park east bench sits on leftover-city grade',
    groundHeight(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z) === CITY_Y);
  ok('E-park east bench is reserved',
    inReserved(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z));
  ok('E-park east bench is a keepout',
    inKeepout(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z));
  ok('reservedOverlap covers the E-park east slat',
    reservedOverlap(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z,
      PARK_BENCH_EE_E_W, PARK_BENCH_EE_E_DEPTH, 0.15));
  ok('tryPlace drops the reserved E-park east bench',
    tryPlace(ctx, PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z) === 0);
  ok('tryPlace does not remap the E-park east bench',
    tryPlace(ctx, PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z) === 0);
  ok('E-park east bench cell is not rejected',
    !gardenBenchRejected(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z));
  ok('E-park east bench footprint is not in the street',
    !streetOverlap(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z,
      PARK_BENCH_EE_E_W, PARK_BENCH_EE_E_DEPTH));
  ok('inGardenBench covers the E-park east plate',
    inGardenBench(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z)
    && inGardenBench(PARK_BENCH_EE_E_X0, PARK_BENCH_EE_E_Z)
    && inGardenBench(PARK_BENCH_EE_E_X1, PARK_BENCH_EE_E_Z));
  ok('E-park east bench does not kiss a garden-path slab',
    !inGardenPathSlab(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z)
    && !inGardenPathSlab(PARK_BENCH_EE_E_X0, PARK_BENCH_EE_E_Z0)
    && !inGardenPathSlab(PARK_BENCH_EE_E_X1, PARK_BENCH_EE_E_Z0)
    && !inGardenPathSlab(PARK_BENCH_EE_E_X0, PARK_BENCH_EE_E_Z1)
    && !inGardenPathSlab(PARK_BENCH_EE_E_X1, PARK_BENCH_EE_E_Z1)
    && !gardenPathSlabOverlap(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z,
      PARK_BENCH_EE_E_W, PARK_BENCH_EE_E_DEPTH, 0));
  ok('E-park east bench does not kiss the EE spine slabs',
    PARK_BENCH_EE_E_Z1 < PARK_WALK_EE_Z0
    && Math.abs(PARK_WALK_EE_Z0 - PARK_BENCH_EE_E_Z - 0.8) < 1e-9);
  ok('E-park east bench does not kiss 347/94.4',
    PARK_BENCH_EE_E_X0 > PARK_BENCH_EE_X1
    && PARK_BENCH_EE_E_Z === PARK_BENCH_EE_Z
    && !gardenBenchRejected(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z)
    && !gardenBenchRejected(PARK_BENCH_EE_X, PARK_BENCH_EE_Z));
  ok('E-park east bench does not kiss 340.5/94.4',
    PARK_BENCH_EE_E_X0 > PARK_BENCH_EE_W_X1
    && PARK_BENCH_EE_E_Z === PARK_BENCH_EE_W_Z
    && !gardenBenchRejected(PARK_BENCH_EE_W_X, PARK_BENCH_EE_W_Z));
  ok('E-park east bench does not kiss west walk 339→345.2 / z=98.5',
    PARK_BENCH_EE_E_X0 > PARK_WALK_EE_W_X1
    && PARK_WALK_EE_W_X1 === 345.2);
  ok('E-park east bench does not kiss east walk 348.8→355 / z=98.5',
    PARK_BENCH_EE_E_Z1 < PARK_WALK_EE_E_Z0
    && PARK_WALK_EE_E_Z0 === 97.7);
  ok('E-park east bench misses warehouse / helipad / 276 park',
    !warehouseOverlap(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z,
      PARK_BENCH_EE_E_W, PARK_BENCH_EE_E_DEPTH, 0.15)
    && !inWarehouseReserved(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z)
    && !inHelipadReserved(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z)
    && PARK_BENCH_EE_E_X0 > POCKET_PARK_X1 && POCKET_PARK_X1 === 284);

  // ---- sit-box is a void; collider ⊆ legs + slats + back -----------------
  ok('E-park east bench back sits inland (−Z) so the seat faces +Z / z=96',
    parts.backs.every((b) => b.z < geom.z)
    && parts.legs.filter((l) => l.sy === geom.backH).every((l) => l.z < geom.z)
    && parts.zBack < parts.zFront);
  ok('E-park east bench slats are the same 40–50 mm kit',
  ...
  ok('E-park east bench one collider per leg / slat / back',
    aabbs.length === meshParts.length);
  ok('E-park east bench colliders are only legs + slats + back',
    aabbs.every((s) => s.part === 'leg' || s.part === 'slat' || s.part === 'back'));
  ok('E-park east bench has no filled sit AABB',
    !aabbs.some((s) => s.y0 >= CITY_Y + PARK_BENCH_EE_E_SEAT_H - 0.02
      && s.sy >= 0.20 && s.sz >= 0.20 && s.sx >= 1.0));
  for (let i = 0; i < meshParts.length; i++) {
    const p = meshParts[i];
    const hit = aabbs[i];
    ok(`eee ${p.id} collider ⊆ part ±0.15`,
      !!hit
    ...
    ok(`eee ${p.id} collider exists`, !!onPart);
  }
  const under = voids.find((v) => v.id === 'gardenBench-under');
  const sit = voids.find((v) => v.id === 'gardenBench-sit');
  ok('E-park east bench ships under-clear + sit voids', !!under && !!sit);
  for (const v of voids) {
    const hit = probeBlocked(shapes, v.x, v.y, v.z, v.probe);
    ok(`eee ${v.id} is flyable`, !hit, hit ? `blocked by ${hit.tag} ${hit.part || hit.type}` : '');
  }
  ok('E-park east sit-box is a void', sit && sit.kind === 'sit'
    && sit.y > CITY_Y + PARK_BENCH_EE_E_SEAT_H);
  ok('E-park east sit-box is in front of the back (+Z of centre, toward z=96)',
    sit && sit.z > geom.z);
  ...
  ok('drop if the kit kisses 353.5/94.4',
    gardenBenchRejected(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z) === false
    && gardenBenchRejected(352.6, 94.4) === true);
  ...
  ok('E-park east bench reuses gardenBenchGeom, no H/EEE/E leftover Geom fork',
    bench.includes('gardenBenchGeom(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z)')
    && bench.includes('gardenBenchParts(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z)')
    && bench.includes('onPavement(PARK_BENCH_EE_E_X, PARK_BENCH_EE_E_Z)')
    ...
    && !/function gardenBenchHGeom/.test(bench)
    && !/gardenBenchHGeom\(/.test(bench)
    && !/function parkBenchEEEGeom/.test(bench)
    && !/parkBenchEEEGeom\(/.test(bench)
    && !/function leftoverLotEGeom/.test(bench)
    && !/leftoverLotEGeom\(/.test(bench)
    && constants.includes('export function gardenBenchGeom')
    && constants.includes('353.5 / 94.4')
    && constants.includes('PARK_BENCH_EE_E_X = 353.5')
    && constants.includes('PARK_BENCH_EE_E_Z = 94.4')
    ...
    && !/gardenBenchHGeom\(/.test(constants)
    && !/export function parkBenchEEEGeom/.test(constants)
    && !/parkBenchEEEGeom\(/.test(constants)
    && !/export function leftoverLotEGeom/.test(constants)
    && !/leftoverLotEGeom\(/.test(constants)
    && !existsSync(join(here, 'landmarks/parkBenchEEE.js'))
    && !existsSync(join(here, 'parkBenchEEE.js'))
  ...
  ok('340.5/94.4 still reuses gardenBenchGeom',
  ...
  ok('gardenPath was not restacked',
    ...
    && !garden.includes('PARK_BENCH_EE_E_'));
  ...
  ok('leftoverLot A/B/C/D/E were not restacked',
    ...
    && !leftover.includes('PARK_BENCH_EE_E_')
  ...
  ok('house was not restacked',
    ...
    && !house.includes('PARK_BENCH_EE_E_'));
  ok('warehouse was not restacked',
    warehouse.includes('warehouseAisleGeom') && warehouse.includes('buildRacks')
    && !warehouse.includes('PARK_BENCH_EE_E_'));
  ok('drop was not restacked',
    drop.includes('dropHoistGeom') && drop.includes('buildWell')
    && !drop.includes('PARK_BENCH_EE_E_'));
  ok('abando was not restacked',
    abando.includes('abandoStairGeom') && abando.includes('buildSilo')
    && !abando.includes('PARK_BENCH_EE_E_'));
  ok('blades.js was not restacked', blades.includes('placeBladePlan')
    && blades.includes('Craft writes the gust')
    && !blades.includes('PARK_BENCH_EE_E_'));
  ok('bay water was not restacked', water.includes('MeshPhysicalMaterial')
    && !water.includes('PARK_BENCH_EE_E_'));
  ok('preview.html was not restacked',
    preview.includes('url=./') && !preview.includes('PARK_BENCH_EE_E_'));
  ok('follow.js was not restacked',
    follow.includes('hauntFollowPath') && !follow.includes('PARK_BENCH_EE_E_'));
  ok('checkpoints.js was not restacked',
    checkpoints.includes('RESTART_OFFSET') && !checkpoints.includes('PARK_BENCH_EE_E_'));
  ok('quad.js GRAVITY stays 9.81',
    /const GRAVITY = 9\.81/.test(quad) && !quad.includes('PARK_BENCH_EE_E_'));
  ok('planting.js was not restacked',
    planting.includes('export function tryPlace')
    && !planting.includes('PARK_BENCH_EE_E_') && !planting.includes('gardenBench'));

  if (fails.length) {
    console.error('[miami-gardenBench-ee-e] FAIL');
    for (const f of fails) console.error('  -', f);
  } else {
    console.log('[miami-gardenBench-ee-e] ok', passedCount, 'checks');
  }
  return {
    passed: fails.length === 0, fails, passedCount,
    placedE: fieldE.placed.length, plannedE,
    x: PARK_BENCH_EE_E_X, z: PARK_BENCH_EE_E_Z,
    x0: PARK_BENCH_EE_E_X0, x1: PARK_BENCH_EE_E_X1,
    yaw: PARK_BENCH_EE_E_YAW,
  };
}

const isMain = typeof process !== 'undefined'
  && process.argv[1] && process.argv[1].endsWith('gardenBenchEEETest.js');
if (isMain) {
  const r = runMiamiGardenBenchEEETests();
