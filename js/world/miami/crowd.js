...
import {
  ...
  WASH_TRAVEL_Z0, WASH_TRAVEL_Z1, WASH_X0,
...
export const LIFEGUARD_SIT_CELLS = LIFEGUARD_CELLS;
export { LIFEGUARD_DECK, LIFEGUARD_SAND_SIT_CELLS };
...
// GAP_X=-501 sidewalk centres: street edge ±1.2 (XS_HALF+1.2). West of leftoverLot A.
export const GAP501_WALK_XS = Object.freeze([-508.7, -493.3]);
export const GAP501_WALK_Z_RUNS = Object.freeze([
  [92, 168],
  [190, 220],
]);
// E-W sidewalks ocean/inland of the z=152 and z=210 mid-rise rows.
// 1.4 m off the plate face so keepout (plate ±0.6) misses. Walk +X.
// Skip GAP_X. West of leftoverLot A / x=240. Miss travel 40.2–47.8
...
// E-W sidewalks ocean/inland of the east z=96 plate at x=210.
// Same 87.6 / 104.4 offsets. East of cinema mass (x1=189). West of abando
// (x0=217), GAP_X=243, leftoverLot A / x=240. Miss travel 40.2–47.8.
export const EAST96_WALK_ZS = ROW96_WALK_ZS;
export const EAST96_WALK_RUNS = Object.freeze([
  [201, 216],
]);
// E-W sidewalks ocean/inland of the z=196 row west of WASH_X0=-480.
// 1.4 m off the plate face (189 / 203). Walk +X. Skip GAP_X.
// Miss Washington travel 176.2–183.8 and Ocean Drive travel 40.2–47.8.
export const ROW196_WALK_ZS = Object.freeze([187.6, 204.4]);
export const ROW196_WALK_RUNS = Object.freeze([
  [-730, -508],
]);
// E-W sidewalk ocean of the z=237 skyline row. 1.4 m off the plate face
// (z0=230 → 228.6). Walk +X. Skip GAP_X. West of leftoverLot A / x=240.
// Miss travel 40.2–47.8 and Washington travel 176.2–183.8. Inland face
// is the z=248 alley — leave it empty.
export const SKYLINE_WALK_ZS = Object.freeze([228.6]);
export const SKYLINE_WALK_RUNS = Object.freeze([
  [-730, -508],
  [-494, -322],
  [-308, -136],
  [-122, 50],
...
function onMidriseWalk(x, z) {
  let onZ = false;
  for (let i = 0; i < MIDRISE_WALK_ZS.length; i++) {
    if (Math.abs(z - MIDRISE_WALK_ZS[i]) <= 0.9) { onZ = true; break; }
  }
  if (!onZ) return false;
...
function onEast96Walk(x, z) {
  let onZ = false;
  for (let i = 0; i < EAST96_WALK_ZS.length; i++) {
    if (Math.abs(z - EAST96_WALK_ZS[i]) <= 0.9) { onZ = true; break; }
  }
  if (!onZ) return false;
  for (let i = 0; i < EAST96_WALK_RUNS.length; i++) {
    const r = EAST96_WALK_RUNS[i];
    if (x >= r[0] && x <= r[1]) return true;
  }
  return false;
}

function onRow196Walk(x, z) {
  if (x >= WASH_X0) return false;
  let onZ = false;
  for (let i = 0; i < ROW196_WALK_ZS.length; i++) {
    if (Math.abs(z - ROW196_WALK_ZS[i]) <= 0.9) { onZ = true; break; }
  }
  if (!onZ) return false;
  for (let i = 0; i < ROW196_WALK_RUNS.length; i++) {
    const r = ROW196_WALK_RUNS[i];
    if (x >= r[0] && x <= r[1]) return true;
  }
  return false;
}

function onSkylineWalk(x, z) {
  let onZ = false;
  for (let i = 0; i < SKYLINE_WALK_ZS.length; i++) {
    if (Math.abs(z - SKYLINE_WALK_ZS[i]) <= 0.9) { onZ = true; break; }
  }
  if (!onZ) return false;
  for (let i = 0; i < SKYLINE_WALK_RUNS.length; i++) {
    const r = SKYLINE_WALK_RUNS[i];
    if (x >= r[0] && x <= r[1]) return true;
  }
  return false;
}
...
export function buildCrowd(ctx) {
  ...
  const nGuardSand = LIFEGUARD_SAND_SIT_CELLS.length;
  const nInland = 36;
  const nLincoln = 40;
  ...
  for (let i = 0; i < nGuardSand; i++) {
    const [x, z] = LIFEGUARD_SAND_SIT_CELLS[i];
    if (npcOffLimits(x, z) || inKeepout(x, z)) continue;
    if (leftoverLotOverlap(x, z, 0.6, 0.6, 0.15)) continue;
    const y = groundHeight(x, z);
    if (y < 0.12) continue;
    actors.push({
      kind: 'guard-sand', i: actors.length, extra: -1,
      x, z, y, dir: 0,
      yaw: 0, speed: 0, phase: hash01(i + 2800, 3) * Math.PI * 2,
      shirt: pick(SHIRT, i + 2800, 5), skin: pick(SKIN, i + 2800, 7),
    });
  }
  ...
  for (let i = 0; i < nGap501; i++) {
    ...
    const dir = hash01(i + 2700, 5) < 0.5 ? 1 : -1;
    if (npcOffLimits(x, z)) continue;
    actors.push({
      kind: 'gap501', i: actors.length, extra: -1, run: runI,
      x, z, y: CITY_Y + 0.06, dir,
      yaw: dir > 0 ? Math.PI / 2 : -Math.PI / 2,
      speed: 1.05 + hash01(i + 2700, 7) * 0.45,
      phase: hash01(i + 2700, 11) * Math.PI * 2,
      shirt: pick(SHIRT, i + 2700, 13), skin: pick(SKIN, i + 2700, 17),
    });
  }
  ...
  for (let i = 0; i < nMidrise; i++) {
    ...
    const dir = hash01(i + 3600, 7) < 0.5 ? 1 : -1;
    if (npcOffLimits(x, z) || !onMidriseWalk(x, z)) continue;
    actors.push({
      kind: 'midrise', i: actors.length, extra: -1, run: runI,
      x, z, y: CITY_Y + 0.06, dir,
      yaw: dir > 0 ? 0 : Math.PI,
      speed: 1.05 + hash01(i + 3600, 11) * 0.45,
      phase: hash01(i + 3600, 13) * Math.PI * 2,
      shirt: pick(SHIRT, i + 3600, 17), skin: pick(SKIN, i + 3600, 19),
    });
  }

  for (let i = 0; i < nRow96; i++) {
    const runI = i % ROW96_WALK_RUNS.length;
    const run = ROW96_WALK_RUNS[runI];
    ...
    actors.push({
      ...
      x, z, y: CITY_Y + 0.06, dir,
      yaw: dir > 0 ? 0 : Math.PI,
      speed: 1.05 + hash01(i + 3800, 11) * 0.45,
      phase: hash01(i + 3800, 13) * Math.PI * 2,
      shirt: pick(SHIRT, i + 3800, 17), skin: pick(SKIN, i + 3800, 19),
    });
  }

  for (let i = 0; i < nEast96; i++) {
    const runI = i % EAST96_WALK_RUNS.length;
    const run = EAST96_WALK_RUNS[runI];
    const x = run[0] + hash01(i + 4000, 3) * (run[1] - run[0]);
    const z = EAST96_WALK_ZS[i % EAST96_WALK_ZS.length]
      + (hash01(i + 4000, 5) - 0.5) * 0.8;
    const dir = hash01(i + 4000, 7) < 0.5 ? 1 : -1;
    if (npcOffLimits(x, z) || !onEast96Walk(x, z) || onCross(x)) continue;
    actors.push({
      kind: 'east96', i: actors.length, extra: -1, run: runI,
      x, z, y: CITY_Y + 0.06, dir,
      yaw: dir > 0 ? 0 : Math.PI,
      speed: 1.05 + hash01(i + 4000, 11) * 0.45,
      phase: hash01(i + 4000, 13) * Math.PI * 2,
      shirt: pick(SHIRT, i + 4000, 17), skin: pick(SKIN, i + 4000, 19),
    });
  }

  for (let i = 0; i < nRow196; i++) {
    const runI = i % ROW196_WALK_RUNS.length;
    const run = ROW196_WALK_RUNS[runI];
    const x = run[0] + hash01(i + 4400, 3) * (run[1] - run[0]);
    const z = ROW196_WALK_ZS[i % ROW196_WALK_ZS.length]
      + (hash01(i + 4400, 5) - 0.5) * 0.8;
    const dir = hash01(i + 4400, 7) < 0.5 ? 1 : -1;
    if (npcOffLimits(x, z) || !onRow196Walk(x, z) || onCross(x)) continue;
    actors.push({
      kind: 'row196', i: actors.length, extra: -1, run: runI,
      x, z, y: CITY_Y + 0.06, dir,
      yaw: dir > 0 ? 0 : Math.PI,
      speed: 1.05 + hash01(i + 4400, 11) * 0.45,
      phase: hash01(i + 4400, 13) * Math.PI * 2,
      shirt: pick(SHIRT, i + 4400, 17), skin: pick(SKIN, i + 4400, 19),
    });
  }

  for (let i = 0; i < nSkyline; i++) {
    const runI = i % SKYLINE_WALK_RUNS.length;
    const run = SKYLINE_WALK_RUNS[runI];
    const x = run[0] + hash01(i + 4500, 3) * (run[1] - run[0]);
    const z = SKYLINE_WALK_ZS[i % SKYLINE_WALK_ZS.length]
      + (hash01(i + 4500, 5) - 0.5) * 0.8;
    const dir = hash01(i + 4500, 7) < 0.5 ? 1 : -1;
    if (npcOffLimits(x, z) || !onSkylineWalk(x, z) || onCross(x)) continue;
    actors.push({
      kind: 'skyline', i: actors.length, extra: -1, run: runI,
      x, z, y: CITY_Y + 0.06, dir,
      yaw: dir > 0 ? 0 : Math.PI,
      speed: 1.05 + hash01(i + 4500, 11) * 0.45,
      phase: hash01(i + 4500, 13) * Math.PI * 2,
      shirt: pick(SHIRT, i + 4500, 17), skin: pick(SKIN, i + 4500, 19),
    });
  }
  ...
  for (let i = 0; i < nHotelPorchSit; i++) {
    ...
    if (npcOffLimits(x, z)) continue;
    actors.push({
      kind: 'porch-sit', i: actors.length, extra: -1,
      x, z, y: CITY_Y + 0.06, dir: 0,
      yaw: hash01(i + 4100, 7) < 0.5 ? 0 : Math.PI,
      speed: 0, phase: hash01(i + 4100, 11) * Math.PI * 2,
      shirt: pick(SHIRT, i + 4100, 13), skin: pick(SKIN, i + 4100, 17),
    });
  }
...
function stepActors(state, dt) {
  ...
  for (let i = 0; i < actors.length; i++) {
    ...
    if (a.kind === 'gap501') {
      a.z += a.dir * a.speed * dt;
      const run = GAP501_WALK_Z_RUNS[a.run] || GAP501_WALK_Z_RUNS[0];
      if (a.z > run[1]) { a.z = run[1]; a.dir = -1; a.yaw = -Math.PI / 2; }
      if (a.z < run[0]) { a.z = run[0]; a.dir = 1; a.yaw = Math.PI / 2; }
      if (npcOffLimits(a.x, a.z)) {
        a.dir *= -1;
    ...
    if (a.kind === 'midrise') {
      a.x += a.dir * a.speed * dt;
      const run = MIDRISE_WALK_RUNS[a.run] || MIDRISE_WALK_RUNS[0];
      if (a.x > run[1]) { a.x = run[1]; a.dir = -1; a.yaw = Math.PI; }
      if (a.x < run[0]) { a.x = run[0]; a.dir = 1; a.yaw = 0; }
      if (npcOffLimits(a.x, a.z) || !onMidriseWalk(a.x, a.z)) {
        a.dir *= -1;
        a.yaw = a.dir > 0 ? 0 : Math.PI;
      }
      continue;
    ...
    if (a.kind === 'row96') {
      ...
      if (a.x > run[1]) { a.x = run[1]; a.dir = -1; a.yaw = Math.PI; }
      if (a.x < run[0]) { a.x = run[0]; a.dir = 1; a.yaw = 0; }
      if (npcOffLimits(a.x, a.z) || !onRow96Walk(a.x, a.z) || inHelipadReserved(a.x, a.z)) {
        a.dir *= -1;
        a.yaw = a.dir > 0 ? 0 : Math.PI;
      }
      continue;
    ...
    if (a.kind === 'east96') {
      a.x += a.dir * a.speed * dt;
      const run = EAST96_WALK_RUNS[a.run] || EAST96_WALK_RUNS[0];
      if (a.x > run[1]) { a.x = run[1]; a.dir = -1; a.yaw = Math.PI; }
      if (a.x < run[0]) { a.x = run[0]; a.dir = 1; a.yaw = 0; }
      if (npcOffLimits(a.x, a.z) || !onEast96Walk(a.x, a.z) || onCross(a.x)) {
        a.dir *= -1;
        a.yaw = a.dir > 0 ? 0 : Math.PI;
      }
      continue;
    ...
    if (a.kind === 'row196') {
      a.x += a.dir * a.speed * dt;
      const run = ROW196_WALK_RUNS[a.run] || ROW196_WALK_RUNS[0];
      if (a.x > run[1]) { a.x = run[1]; a.dir = -1; a.yaw = Math.PI; }
      if (a.x < run[0]) { a.x = run[0]; a.dir = 1; a.yaw = 0; }
      if (npcOffLimits(a.x, a.z) || !onRow196Walk(a.x, a.z) || onCross(a.x)) {
        a.dir *= -1;
        a.yaw = a.dir > 0 ? 0 : Math.PI;
      }
      continue;
    ...
    if (a.kind === 'skyline') {
      a.x += a.dir * a.speed * dt;
      const run = SKYLINE_WALK_RUNS[a.run] || SKYLINE_WALK_RUNS[0];
      if (a.x > run[1]) { a.x = run[1]; a.dir = -1; a.yaw = Math.PI; }
      if (a.x < run[0]) { a.x = run[0]; a.dir = 1; a.yaw = 0; }
      if (npcOffLimits(a.x, a.z) || !onSkylineWalk(a.x, a.z) || onCross(a.x)) {
        a.dir *= -1;
        a.yaw = a.dir > 0 ? 0 : Math.PI;
      }
      continue;
...
function stampAll(state) {
  ...
  for (let i = 0; i < actors.length; i++) {
    ...
    const crouch = (a.kind === 'sit' || a.kind === 'guard' || a.kind === 'guard-sand'
      ...
      || a.kind === 'arcade-sit' || a.kind === 'porch-sit') ? 0.28
    ...
    _dummy.scale.set(1, (a.kind === 'sit' || a.kind === 'guard' || a.kind === 'guard-sand'
      || a.kind === 'lincoln-sit'
      || a.kind === 'lummus-sit' || a.kind === 'towel-sit'
      || a.kind === 'arcade-sit' || a.kind === 'porch-sit') ? 0.72 : 1, 1);
    _dummy.updateMatrix();
