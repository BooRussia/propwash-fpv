...
export function runMiamiParkWalkHHTests() {
  fails.length = 0;
  passedCount = 0;
  ...
  const ff = gardenPathGeom(PARK_WALK_FF_X, PARK_WALK_FF_Z);
  const gg = gardenPathGeom(PARK_WALK_GG_X, PARK_WALK_GG_Z);
  const geom = gardenPathGeom(PARK_WALK_HH_X, PARK_WALK_HH_Z);
  ...
  const ffSlabs = gardenPathSlabs(ff);
  const ggSlabs = gardenPathSlabs(gg);
  ...
  // ---- signed corridor (Desi); do not invent or slide --------------------
  ok('walk walks 390→406 in x', PARK_WALK_HH_X0 === 390 && PARK_WALK_HH_X1 === 406);
  ok('walk centre is signed z=96', PARK_WALK_HH_Z === 96
    && PARK_WALK_HH_X === 398);
  ok('walk width is 1.6 m (z 95.2–96.8)',
    PARK_WALK_HH_W === 1.6 && PARK_WALK_HH_Z0 === 95.2 && PARK_WALK_HH_Z1 === 96.8
    && Math.abs(PARK_WALK_HH_Z1 - PARK_WALK_HH_Z0 - PARK_WALK_HH_W) < 1e-9
    && PARK_WALK_HH_W === GARDEN_PATH_W
    && PARK_WALK_HH_W === PARK_WALK_W
    && PARK_WALK_HH_W === PARK_WALK_GG_W
    && PARK_WALK_HH_W === PARK_WALK_FF_W
    && PARK_WALK_HH_W === PARK_WALK_EE_W);
  ok('walk length is 16 m (16 × 1.6 on the 398/96 hull)', PARK_WALK_HH_LEN === 16
    && Math.abs(PARK_WALK_HH_X1 - PARK_WALK_HH_X0 - PARK_WALK_HH_LEN) < 1e-9
    && PARK_WALK_HH_LEN === GARDEN_PATH_LEN
    && PARK_WALK_HH_LEN === PARK_WALK_GG_LEN);
  ok('6 m inland of leftoverLot H (H z1=90)',
    LEFTOVER_LOT_H_Z1 === 90 && PARK_WALK_HH_Z === 96
    && Math.abs(PARK_WALK_HH_Z - LEFTOVER_LOT_H_Z1 - 6) < 1e-9);
  ok('geom matches signed constants',
    geom.x0 === 390 && geom.x1 === 406 && geom.z === 96
    && geom.z0 === 95.2 && geom.z1 === 96.8 && geom.w === 1.6
    && geom.x === 398 && geom.len === 16);
  ok('x/z were not invented or slid',
    geom.x === PARK_WALK_HH_X && geom.z === PARK_WALK_HH_Z
    && PARK_WALK_HH_X0 === 390 && PARK_WALK_HH_X1 === 406 && PARK_WALK_HH_Z === 96);
  ok('last slab stays inside 406',
    PARK_WALK_HH_X1 === 406
    && slabs.every((s) => s.x1 <= 406 + 1e-9)
    && lastSlab && lastSlab.x1 <= 406 + 1e-9);
  ok('did not grow past 406',
    geom.x1 === 406 && PARK_WALK_HH_X1 === 406
    && slabs.every((s) => s.x1 <= PARK_WALK_HH_X1 + 1e-9));
  ok('sits on the signed 398/96 hull',
    geom.x0 === POCKET_PARK_H_X0 && geom.x1 === POCKET_PARK_H_X1
    && geom.z >= POCKET_PARK_H_Z0 && geom.z <= POCKET_PARK_H_Z1
    && parkH.x === 398 && parkH.z === 96
    && parkH.x0 === 390 && parkH.x1 === 406);
  ok('does not merge with PARK_WALK_GG_X1=389 (1 m west gap, same z)',
    PARK_WALK_GG_X1 === 389 && PARK_WALK_HH_X0 === 390
    && PARK_WALK_HH_X0 === PARK_WALK_GG_X1 + 1
    && PARK_WALK_GG_Z === PARK_WALK_HH_Z
    && PARK_WALK_GG_Z0 === PARK_WALK_HH_Z0
    && PARK_WALK_GG_Z1 === PARK_WALK_HH_Z1
    && geom.x0 > gg.x1
    && !ggSlabs.some((s) => s.x1 > geom.x0 - 1e-9 && s.x0 < geom.x1 + 1e-9
      && s.z1 > geom.z0 - 1e-9 && s.z0 < geom.z1 + 1e-9));

  ok('walk is not pavement', !onPavement(PARK_WALK_HH_X, PARK_WALK_HH_Z));
  ok('walk is not boardwalk', !onBoardwalk(PARK_WALK_HH_X, PARK_WALK_HH_Z));
  ok('walk is not roadway', !onRoadway(PARK_WALK_HH_Z));
  ok('walk is not a cross-street', !onCrossStreet(PARK_WALK_HH_X, PARK_WALK_HH_Z));
  ok('walk is not a sidewalk slab', !onSidewalk(PARK_WALK_HH_X, PARK_WALK_HH_Z));
  ok('walk sits on leftover-city grade',
    groundHeight(PARK_WALK_HH_X, PARK_WALK_HH_Z) === CITY_Y);
  ok('walk is reserved', inReserved(PARK_WALK_HH_X, PARK_WALK_HH_Z));
  ok('walk is a keepout', inKeepout(PARK_WALK_HH_X, PARK_WALK_HH_Z));
  ok('reservedOverlap covers the signed walk',
    reservedOverlap(PARK_WALK_HH_X, PARK_WALK_HH_Z, PARK_WALK_HH_LEN, PARK_WALK_HH_W, 0.15));
  ok('tryPlace drops the reserved walk',
    tryPlace(ctx, PARK_WALK_HH_X, PARK_WALK_HH_Z) === 0);
  ok('tryPlace does not remap the walk',
    tryPlace(ctx, PARK_WALK_HH_X, PARK_WALK_HH_Z) === 0);
  ok('signed cell is not rejected', !gardenPathRejected(PARK_WALK_HH_X, PARK_WALK_HH_Z));
  ok('gardenPathRejected(398, 96) stays false after the wire-up',
    !gardenPathRejected(398, 96)
    && PARK_WALK_HH_X === 398 && PARK_WALK_HH_Z === 96);
  ok('walk footprint is not in the street',
    !streetOverlap(PARK_WALK_HH_X, PARK_WALK_HH_Z, PARK_WALK_HH_LEN, PARK_WALK_HH_W));
  ok('inGardenPath covers the signed H-park spine',
    inGardenPath(PARK_WALK_HH_X, PARK_WALK_HH_Z)
    && inGardenPath(PARK_WALK_HH_X0, PARK_WALK_HH_Z)
    && inGardenPath(PARK_WALK_HH_X1, PARK_WALK_HH_Z));

  // ---- 276 park / walks / A–H / benches / pergola / EE / FF / GG stay put ----
  ...
  ok('E-park west walk stays 339→345.2 / z=98.5',
    PARK_WALK_EE_W_X0 === 339 && PARK_WALK_EE_W_X1 === 345.2
    && PARK_WALK_EE_W_Z === 98.5);
  ok('E-park east walk stays 348.8→355 / z=98.5',
  ...
  ok('garden path was not slid onto z=96',
    garden.z === 84 && garden.x1 === 284 && geom.z === 96 && geom.x0 === 390);
  ok('EE spine was not slid onto 390→406',
    ee.x0 === 339 && ee.x1 === 355 && ee.z === 96
    && geom.x0 === 390 && geom.x1 === 406 && geom.z === 96);
  ok('FF spine stays 356→372 / z=96',
    ...
    && PARK_WALK_FF_Z0 === 95.2 && PARK_WALK_FF_Z1 === 96.8
    && ff.x0 === 356 && ff.x1 === 372 && ff.z === 96);
  ok('FF spine was not slid onto 390→406',
    ff.x0 === 356 && ff.x1 === 372 && ff.z === 96
    && geom.x0 === 390 && geom.x1 === 406 && geom.z === 96);
  ok('GG spine stays 373→389 / z=96',
    ...
    && PARK_WALK_GG_Z0 === 95.2 && PARK_WALK_GG_Z1 === 96.8
    && gg.x0 === 373 && gg.x1 === 389 && gg.z === 96);
  ok('GG spine was not slid onto 390→406',
    gg.x0 === 373 && gg.x1 === 389 && gg.z === 96
    && geom.x0 === 390 && geom.x1 === 406 && geom.z === 96);
  ...
  ok('F park stays 364/96, 16×8 (356–372 × 92–100)',
  ...
  ok('G park stays 381/96, 16×8 (373–389 × 92–100)',
  ...
  ok('H park stays 398/96, 16×8 (390–406 × 92–100)',
  ...
  ok('leftoverLot A/B/C/D/E/F/G/H geometry was not slid',
  ...
  ok('leftoverLot H reserved z1+1.4=91.4 vs walk z0=95.2',
    LEFTOVER_LOT_H_Z1 + 1.4 === 91.4 && PARK_WALK_HH_Z0 === 95.2
    && PARK_WALK_HH_Z0 > LEFTOVER_LOT_H_Z1 + 1.4);
  ok('leftoverLotOverlap of H reserved is 0',
    leftoverLotOverlap(PARK_WALK_HH_X, PARK_WALK_HH_Z, PARK_WALK_HH_LEN, PARK_WALK_HH_W, 0.15) === false);
  ok('does not overlap leftoverLot A–H reserved',
    !leftoverLotOverlap(PARK_WALK_HH_X, PARK_WALK_HH_Z, PARK_WALK_HH_LEN, PARK_WALK_HH_W, 0.15)
    && leftoverLotOverlap(PARK_WALK_HH_X, PARK_WALK_HH_Z, PARK_WALK_HH_LEN, PARK_WALK_HH_W, 0.15) === false
    && !inLeftoverLotReserved(PARK_WALK_HH_X, PARK_WALK_HH_Z)
    && !inLeftoverLotReserved(PARK_WALK_HH_X0, PARK_WALK_HH_Z)
    && !inLeftoverLotReserved(PARK_WALK_HH_X1, PARK_WALK_HH_Z)
    && !inLeftoverLotReserved(PARK_WALK_HH_X, PARK_WALK_HH_Z0));
  ok('does not overlap warehouse reserved',
    !warehouseOverlap(PARK_WALK_HH_X, PARK_WALK_HH_Z, PARK_WALK_HH_LEN, PARK_WALK_HH_W, 0.15)
    && !inWarehouseReserved(PARK_WALK_HH_X, PARK_WALK_HH_Z));
  ok('does not sit on helipad E',
    !inHelipadReserved(PARK_WALK_HH_X, PARK_WALK_HH_Z)
    && PARK_WALK_HH_X1 < 408);
  ok('does not sit on garden path 268→284 / z=84',
    ...
    && !gardenSlabs.some((s) => s.x1 > geom.x0 - 1e-9 && s.x0 < geom.x1 + 1e-9
      && s.z1 > geom.z0 - 1e-9 && s.z0 < geom.z1 + 1e-9));
  ok('does not sit on 276 park hull (x1=284 < 390)',
    geom.x0 > POCKET_PARK_X1
    && POCKET_PARK_X1 === 284 && PARK_WALK_HH_X0 === 390
  ...
  ok('tryPlace still drops leftoverLot A–H',
  ...
  ok('tryPlace still drops the EE spine',
    tryPlace(ctx, PARK_WALK_EE_X, PARK_WALK_EE_Z) === 0);
  ok('tryPlace still drops the FF spine',
    tryPlace(ctx, PARK_WALK_FF_X, PARK_WALK_FF_Z) === 0);
  ok('tryPlace still drops the GG spine',
    tryPlace(ctx, PARK_WALK_GG_X, PARK_WALK_GG_Z) === 0);
  ...
  // ---- drop if it kisses leftoverLot H / helipad / warehouse / 276 ------
  ok('drop if the kit sits on leftoverLot H',
    gardenPathRejected(LEFTOVER_LOT_H_X, LEFTOVER_LOT_H_Z) === true);
  ok('drop if the kit sits on leftoverLot G',
    gardenPathRejected(LEFTOVER_LOT_G_X, LEFTOVER_LOT_G_Z) === true);
  ok('drop if the kit sits on leftoverLot F',
  ...
  ok('drop if the kit kisses GG spine slabs (G-park hull merge)',
    gardenPathRejected(PARK_WALK_GG_X, PARK_WALK_GG_Z + 1) === true);
  ok('drop pavement / streetOverlap, never nudge',
    gardenPathRejected(0, 27) === true && gardenPathRejected(57, 80) === true);

  // ---- shared slab kit; no parkWalkHHGeom / gardenPathHGeom fork ---------
  ...
  ok('H-park spine reuses gardenPathGeom, same slab kit',
  ...
  ok('slabs stay inside the signed walk',
    slabs.every((s) => s.x0 >= PARK_WALK_HH_X0 - 1e-9
      && s.x1 <= PARK_WALK_HH_X1 + 1e-9
      && s.z0 >= PARK_WALK_HH_Z0 - 1e-9
      && s.z1 <= PARK_WALK_HH_Z1 + 1e-9));
  ...
  ok('no filled path AABB',
    !aabbs.some((s) => s.sx >= PARK_WALK_HH_LEN - 0.4
      && s.sz >= PARK_WALK_HH_W - 0.4 && s.sy >= 0.4));
  ok('H-park spine AABB flag is false',
    PARK_WALK_HH_AABB === false && GARDEN_PATH_AABB === false);
  ...
  ok('one grass hull covers the signed walk',
    hull.x0 === PARK_WALK_HH_X0 && hull.x1 === PARK_WALK_HH_X1
    && hull.z0 === PARK_WALK_HH_Z0 && hull.z1 === PARK_WALK_HH_Z1
    && hull.y0 === CITY_Y);
  ...
  ok('grass hull area is width × length',
    Math.abs(hullArea(hull) - PARK_WALK_HH_LEN * PARK_WALK_HH_W) < 1e-9);
  ok('walk eats ~26 m²',
    Math.abs(PARK_WALK_HH_LEN * PARK_WALK_HH_W - 25.6) < 1e-9);
  ok('no grass-hull AABB in the collider bag',
    !aabbs.some((s) => Math.abs(s.sx - PARK_WALK_HH_LEN) < 0.2
      && Math.abs(s.sz - PARK_WALK_HH_W) < 0.2));
  ...
  // ---- H leftover ~10k after the spine; do not drop to 8–11k ------------
  ok('H park hull is not rejected by the spine',
  ...
  ok('H planned grid stays empty-hull 12800 (cover not raised)',
  ...
  ok('did not backfill slabs to hit empty-hull 12800',
    ...
    && fieldH.cells.some((c) => inGardenPath(c.x, c.z)));
  ok('no H blade on the spine / lot H / helipad / warehouse',
    fieldH.placed.every((p) => !pocketParkDrop(p.x, p.z)
      && !inGardenPath(p.x, p.z)
      && !inLeftoverLotReserved(p.x, p.z)
      && !inWarehouseReserved(p.x, p.z)
      && !inHelipadReserved(p.x, p.z)));
  ok('G leftover stays 8000–11000 after the H spine',
    ...
    && fieldG.placed.length < 12000,
    `placedG=${fieldG.placed.length} plannedG=${plannedG}`);
  ok('F leftover stays 8000–11000 after the H spine',
    ...
    && fieldF.placed.length < 12000,
    `placedF=${fieldF.placed.length} plannedF=${plannedF}`);
  ok('E leftover stays 8000–11000 after the H spine',
  ...
  const ffTest = readFileSync(join(here, 'parkWalkFFTest.js'), 'utf8');
  const ggTest = readFileSync(join(here, 'parkWalkGGTest.js'), 'utf8');
  ...
  ok('H-park spine reuses gardenPathGeom, no parkWalkHHGeom fork',
    gardenSrc.includes('gardenPathGeom(PARK_WALK_HH_X, PARK_WALK_HH_Z)')
    && gardenSrc.includes('gardenPathRejected(PARK_WALK_HH_X, PARK_WALK_HH_Z)')
    && gardenSrc.includes('onPavement(PARK_WALK_HH_X, PARK_WALK_HH_Z)')
    && constants.includes('export function gardenPathGeom')
    && constants.includes('390→406')
    && constants.includes('Does not merge with PARK_WALK_GG_X1=389')
    ...
    && !/export function parkWalkEEWGeom/.test(constants)
    && !/export function parkWalkEEEGeom/.test(constants)
    && !/export function parkWalkFFGeom/.test(constants)
    ...
    && !/export function parkWalkFGeom/.test(constants)
    ...
    && !/function parkWalkEEWGeom/.test(gardenSrc)
    && !/function parkWalkEEEGeom/.test(gardenSrc)
    && !/function parkWalkFFGeom/.test(gardenSrc)
    && !/function parkWalkGGGeom/.test(gardenSrc)
    && !/function parkWalkHHGeom/.test(gardenSrc)
    && !/function gardenPathBGeom/.test(gardenSrc)
    && !/function gardenPathFGeom/.test(gardenSrc)
    && !/function gardenPathGGeom/.test(gardenSrc)
    && !/function gardenPathHGeom/.test(gardenSrc)
    && !/function parkWalkFGeom/.test(gardenSrc)
    && !/parkWalkGeom\(/.test(gardenSrc)
    && !/parkWalkEGeom\(/.test(gardenSrc)
    && !/parkWalkE2Geom\(/.test(gardenSrc)
    && !/parkWalkEEGeom\(/.test(gardenSrc)
    && !/parkWalkEEWGeom\(/.test(gardenSrc)
    && !/parkWalkEEEGeom\(/.test(gardenSrc)
    && !/parkWalkFFGeom\(/.test(gardenSrc)
    && !/parkWalkGGGeom\(/.test(gardenSrc)
    && !/parkWalkHHGeom\(/.test(gardenSrc)
    && !/gardenPathBGeom\(/.test(gardenSrc)
    && !/gardenPathFGeom\(/.test(gardenSrc)
    && !/gardenPathGGeom\(/.test(gardenSrc)
    && !/gardenPathHGeom\(/.test(gardenSrc)
    && !/parkWalkFGeom\(/.test(gardenSrc)
    && !/leftoverLotDirtGeom\(/.test(gardenSrc));
  ok('did not ship parkWalkHH.js / gardenPathH.js',
    ...
    && !existsSync(join(here, 'landmarks/parkWalkEEE.js'))
    && !existsSync(join(here, 'landmarks/parkWalkFF.js'))
    && !existsSync(join(here, 'landmarks/parkWalkGG.js'))
    && !existsSync(join(here, 'landmarks/parkWalkHH.js'))
    ...
    && !existsSync(join(here, 'parkWalkHH.js')));
  ok('index keeps the shared gardenPath builder',
    ...
    && !index.includes('buildParkWalkFF')
    && !index.includes('buildParkWalkGG')
    && !index.includes('buildParkWalkHH')
    && !index.includes('parkWalkGeom')
    && !index.includes('parkWalkEGeom')
    && !index.includes('parkWalkEEGeom')
    && !index.includes('parkWalkFFGeom')
    && !index.includes('parkWalkGGGeom')
    && !index.includes('parkWalkHHGeom'));
  ok('no custom mat',
  ...
  ok('276 walks were not restacked',
    ...
    && !westTest.includes('PARK_WALK_HH_') && !eastTest.includes('PARK_WALK_HH_')
  ...
  ok('EE spine was not restacked',
    gardenSrc.includes('339→355') && gardenSrc.includes('PARK_WALK_EE_X')
    && eeTest.includes('PARK_WALK_EE_X0 === 339')
    && eeTest.includes('PARK_WALK_EE_X1 === 355')
    && constants.includes('PARK_WALK_EE_X0 = 339')
    && constants.includes('PARK_WALK_EE_X1 = 355')
    && constants.includes('PARK_WALK_EE_Z = 96'));
  ok('FF spine was not restacked',
    gardenSrc.includes('356→372') && gardenSrc.includes('PARK_WALK_FF_X')
    && ffTest.includes('PARK_WALK_FF_X0 === 356')
    && ffTest.includes('PARK_WALK_FF_X1 === 372')
    && constants.includes('PARK_WALK_FF_X0 = 356')
    && constants.includes('PARK_WALK_FF_X1 = 372')
    && constants.includes('PARK_WALK_FF_Z = 96'));
  ok('GG spine was not restacked',
    gardenSrc.includes('373→389') && gardenSrc.includes('PARK_WALK_GG_X')
    && ggTest.includes('PARK_WALK_GG_X0 === 373')
    && ggTest.includes('PARK_WALK_GG_X1 === 389')
  ...
  ok('leftoverLot A–H were not restacked',
    ...
    && constants.includes('347/84') && constants.includes('364/84')
    && constants.includes('381/84') && constants.includes('398/84')
  ...
  ok('no photo-mode / ACES / bloom / SSAO / colony HUD / Starship / Shackleton',
    !constants.includes('photo-mode') && !constants.includes('ACES')
    && !constants.includes('bloom') && !constants.includes('SSAO')
    && !constants.includes('colony HUD') && !constants.includes('Starship')
    && !constants.includes('Shackleton')
    && !gardenSrc.includes('photo-mode') && !gardenSrc.includes('ACES')
    && !gardenSrc.includes('SSAO') && !gardenSrc.includes('Shackleton'));
  ok('no Selo / leftoverLotDirtGeom fork',
    !constants.includes('Selo') && !gardenSrc.includes('Selo')
    && !gardenSrc.includes('190k')
    && !/leftoverLotDirtGeom\(/.test(constants)
    && !/leftoverLotDirtGeom\(/.test(gardenSrc));

  if (fails.length) {
    console.error('[miami-parkWalkHH] FAIL');
    for (const f of fails) console.error('  -', f);
  } else {
    console.log('[miami-parkWalkHH] ok', passedCount, 'checks',
      `lastSlab.x1=${lastSlab.x1} placedH=${fieldH.placed.length}/${plannedH} placedG=${fieldG.placed.length} placedF=${fieldF.placed.length} placedE=${fieldE.placed.length} placed276=${field276.placed.length}`);
  }
  return {
...
const isMain = typeof process !== 'undefined'
  && process.argv[1] && process.argv[1].endsWith('parkWalkHHTest.js');
if (isMain) {
  const r = runMiamiParkWalkHHTests();
