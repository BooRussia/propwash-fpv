// Headless checks for the Miami east park walk (garden-path slab kit).
// No three.js, no game state. Not a haunt. Not leftoverLot. Not a
// leftoverGrass / pocket-park / garden-path / west-walk / pergola restack.
//
...
export function runMiamiParkWalkETests() {
  fails.length = 0;
  passedCount = 0;
  ...
  const geom = gardenPathGeom(PARK_WALK_E_X, PARK_WALK_E_Z);
  ...
  ok('walk walks 277.8→284 in x', PARK_WALK_E_X0 === 277.8 && PARK_WALK_E_X1 === 284);
  ok('walk centre is signed z=94', PARK_WALK_E_Z === 94
    && Math.abs(PARK_WALK_E_X - 280.9) < 1e-9);
  ok('walk width is 1.6 m (z 93.2–94.8)',
    PARK_WALK_E_W === 1.6 && PARK_WALK_E_Z0 === 93.2 && PARK_WALK_E_Z1 === 94.8
    && Math.abs(PARK_WALK_E_Z1 - PARK_WALK_E_Z0 - PARK_WALK_E_W) < 1e-9
    && PARK_WALK_E_W === GARDEN_PATH_W
    && PARK_WALK_E_W === PARK_WALK_W);
  ok('walk length is 6.2 m (mirror of 268→274.2)', PARK_WALK_E_LEN === 6.2
    && Math.abs(PARK_WALK_E_X1 - PARK_WALK_E_X0 - PARK_WALK_E_LEN) < 1e-9
    && PARK_WALK_E_LEN === PARK_WALK_LEN);
  ok('starts 1.8 m east of 276',
    Math.abs(PARK_WALK_E_X0 - (276 + 1.8)) < 1e-9 && PARK_WALK_E_X0 === 277.8);
  ok('geom matches signed constants',
    geom.x0 === 277.8 && geom.x1 === 284 && geom.z === 94
    && geom.z0 === 93.2 && geom.z1 === 94.8 && geom.w === 1.6
    && Math.abs(geom.x - 280.9) < 1e-9 && geom.len === 6.2);
  ok('x/z were not invented or slid',
    geom.x === PARK_WALK_E_X && geom.z === PARK_WALK_E_Z
    && PARK_WALK_E_X0 === 277.8 && PARK_WALK_E_X1 === 284 && PARK_WALK_E_Z === 94);

  ok('277.16 is 276+1.16, the Z number reused as X, not post x1',
    Math.abs((276 + GATE_HALF_Z) - 277.16) < 1e-9
    && GATE_HALF_Z === 1.16
    && PARK_WALK_E_X0 > (276 + GATE_HALF_Z)
    && PARK_WALK_E_X0 > 277.15);
  ok('post x1 is 276+GATE_HALF_X, not 276+1.16',
    Math.abs((PARK_PERGOLA_X + GATE_HALF_X) - 277.15) < 1e-9
    && GATE_HALF_X === 1.15 && GATE_HALF_X !== GATE_HALF_Z
    && Math.abs(PARK_PERGOLA_X1 - 277.15) < 1e-9);
  ok('walk does not extend into the sash',
    PARK_WALK_E_X0 > sash.x1
    && PARK_WALK_E_X0 > PARK_PERGOLA_X1
    && PARK_WALK_E_X0 > gate.x + gate.halfX);
  ok('pergola posts sit on x=276, Z half-span 1.16 (z 92.84–95.16)',
    ...
    && gate.x === 276);
  ok('last slab stays inside 284; leftoverLot B starts 288',
    PARK_WALK_E_X1 === 284 && LEFTOVER_LOT_B_X0 === 288
    && PARK_WALK_E_X1 < LEFTOVER_LOT_B_X0
    && slabs.every((s) => s.x1 <= 284 + 1e-9));
  ok('did not grow past 284',
    geom.x1 === 284 && PARK_WALK_E_X1 === 284
    && slabs.every((s) => s.x1 <= PARK_WALK_E_X1 + 1e-9));

  ok('walk is not pavement', !onPavement(PARK_WALK_E_X, PARK_WALK_E_Z));
  ok('walk is not boardwalk', !onBoardwalk(PARK_WALK_E_X, PARK_WALK_E_Z));
  ok('walk is not roadway', !onRoadway(PARK_WALK_E_Z));
  ok('walk is not a cross-street', !onCrossStreet(PARK_WALK_E_X, PARK_WALK_E_Z));
  ok('walk is not a sidewalk slab', !onSidewalk(PARK_WALK_E_X, PARK_WALK_E_Z));
  ok('walk sits on leftover-city grade',
    groundHeight(PARK_WALK_E_X, PARK_WALK_E_Z) === CITY_Y);
  ok('walk is reserved', inReserved(PARK_WALK_E_X, PARK_WALK_E_Z));
  ok('walk is a keepout', inKeepout(PARK_WALK_E_X, PARK_WALK_E_Z));
  ok('reservedOverlap covers the signed walk',
    reservedOverlap(PARK_WALK_E_X, PARK_WALK_E_Z, PARK_WALK_E_LEN, PARK_WALK_E_W, 0.15));
  ok('tryPlace drops the reserved walk',
    tryPlace(ctx, PARK_WALK_E_X, PARK_WALK_E_Z) === 0);
  ok('tryPlace does not remap the walk',
    tryPlace(ctx, PARK_WALK_E_X, PARK_WALK_E_Z) === 0);
  ok('signed cell is not rejected', !gardenPathRejected(PARK_WALK_E_X, PARK_WALK_E_Z));
  ok('walk footprint is not in the street',
    !streetOverlap(PARK_WALK_E_X, PARK_WALK_E_Z, PARK_WALK_E_LEN, PARK_WALK_E_W));
  ok('inGardenPath covers the signed east walk',
    inGardenPath(PARK_WALK_E_X, PARK_WALK_E_Z)
    && inGardenPath(PARK_WALK_E_X0, PARK_WALK_E_Z)
    && inGardenPath(PARK_WALK_E_X1, PARK_WALK_E_Z));

  // ---- west walk / garden path / benches / pergola / lots stay put -------
  ...
  ok('garden path was not slid onto z=94',
    garden.z === 84 && garden.x1 === 284 && geom.z === 94 && geom.x0 === 277.8);
  ok('west walk was not slid onto 277.8→284',
    west.x0 === 268 && west.x1 === 274.2 && west.z === 94
    && geom.x0 === 277.8 && geom.x1 === 284);
  ...
  ok('does not overlap leftoverLot A/B/C/D reserved',
    !leftoverLotOverlap(PARK_WALK_E_X, PARK_WALK_E_Z, PARK_WALK_E_LEN, PARK_WALK_E_W, 0.15)
    && !inLeftoverLotReserved(PARK_WALK_E_X, PARK_WALK_E_Z)
    && !inLeftoverLotReserved(PARK_WALK_E_X0, PARK_WALK_E_Z)
    && !inLeftoverLotReserved(PARK_WALK_E_X1, PARK_WALK_E_Z));
  ...
  // ---- shared slab kit; no gardenPathBGeom / parkWalkEGeom fork ----------
  ...
  ok('east reuses gardenPathGeom, same slab kit',
    geom.w === garden.w && geom.h === garden.h && geom.y0 === garden.y0
    && geom.w === GARDEN_PATH_W && geom.w === west.w
    && geom.len === west.len);
  ...
  ok('slabs stay inside the signed walk',
    slabs.every((s) => s.x0 >= PARK_WALK_E_X0 - 1e-9
      && s.x1 <= PARK_WALK_E_X1 + 1e-9
      && s.z0 >= PARK_WALK_E_Z0 - 1e-9
      && s.z1 <= PARK_WALK_E_Z1 + 1e-9));
  ...
  ok('no filled path AABB',
    !aabbs.some((s) => s.sx >= PARK_WALK_E_LEN - 0.4
      && s.sz >= PARK_WALK_E_W - 0.4 && s.sy >= 0.4));
  ok('east park walk AABB flag is false',
    PARK_WALK_E_AABB === false && GARDEN_PATH_AABB === false);
  ...
  ok('walk air void does not reach the sash',
    air.x0 > sash.x1 && air.x0 === PARK_WALK_E_X0);

  // ---- one grass hull at grade; not per-blade colliders (Sylva) ---------
  ok('one grass hull covers the signed walk',
    hull.x0 === PARK_WALK_E_X0 && hull.x1 === PARK_WALK_E_X1
    && hull.z0 === PARK_WALK_E_Z0 && hull.z1 === PARK_WALK_E_Z1
    && hull.y0 === CITY_Y);
  ...
  ok('grass hull area is width × length',
    Math.abs(hullArea(hull) - PARK_WALK_E_LEN * PARK_WALK_E_W) < 1e-9);
  ok('no grass-hull AABB in the collider bag',
    !aabbs.some((s) => Math.abs(s.sx - PARK_WALK_E_LEN) < 0.2
      && Math.abs(s.sz - PARK_WALK_E_W) < 0.2));
  ...
  ok('east walk reuses gardenPathGeom, no parkWalkEGeom fork',
    gardenSrc.includes('gardenPathGeom(PARK_WALK_E_X, PARK_WALK_E_Z)')
    && gardenSrc.includes('gardenPathRejected(PARK_WALK_E_X, PARK_WALK_E_Z)')
    && gardenSrc.includes('onPavement(PARK_WALK_E_X, PARK_WALK_E_Z)')
    && constants.includes('export function gardenPathGeom')
    && constants.includes('277.8→284')
  ...
  ok('did not ship parkWalkE.js / gardenPathE.js',
    ...
    && !existsSync(join(here, 'parkWalkE.js')));
  ...
  ok('kit is Tiny Glade flagstones + one grass hull',
    ...
    && gardenSrc.includes('277.8') && gardenSrc.includes('Desi')
  ...
  ok('west park walk 268→274.2 / z=94 was not restacked',
    ...
    && !westTest.includes('PARK_WALK_E_') && !westTest.includes('277.8')
  ...
  if (fails.length) {
    console.error('[miami-parkWalkE] FAIL');
    for (const f of fails) console.error('  -', f);
  } else {
    console.log('[miami-parkWalkE] ok', passedCount, 'checks');
  }
  return { passed: fails.length === 0, fails, passedCount };
}

const isMain = typeof process !== 'undefined'
  && process.argv[1] && process.argv[1].endsWith('parkWalkETest.js');
if (isMain) {
  const r = runMiamiParkWalkETests();
