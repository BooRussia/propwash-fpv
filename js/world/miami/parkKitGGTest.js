// Headless checks for the Miami G-park kit (364 kit +17 m).
// One file for the rest of the live F-park kit on the 381/96 hull.
...
import {
  ...
  PARK_BENCH_GG_X, PARK_BENCH_GG_Z, PARK_BENCH_GG_YAW, PARK_BENCH_GG_W,
  PARK_BENCH_GG_DEPTH, PARK_BENCH_GG_SEAT_H, PARK_BENCH_GG_BACK_H,
  PARK_BENCH_GG_UNDER_CLEAR,
  PARK_BENCH_GG_X0, PARK_BENCH_GG_X1, PARK_BENCH_GG_Z0, PARK_BENCH_GG_Z1,
  PARK_BENCH_GG_W_X, PARK_BENCH_GG_W_Z, PARK_BENCH_GG_W_YAW, PARK_BENCH_GG_W_W,
  PARK_BENCH_GG_W_X0, PARK_BENCH_GG_W_X1, PARK_BENCH_GG_W_Z0, PARK_BENCH_GG_W_Z1,
  PARK_BENCH_GG_E_X, PARK_BENCH_GG_E_Z, PARK_BENCH_GG_E_YAW, PARK_BENCH_GG_E_W,
  PARK_BENCH_GG_E_X0, PARK_BENCH_GG_E_X1, PARK_BENCH_GG_E_Z0, PARK_BENCH_GG_E_Z1,
  ...
  PARK_WALK_GG_W_X0, PARK_WALK_GG_W_X1, PARK_WALK_GG_W_Z, PARK_WALK_GG_W_W,
  PARK_WALK_GG_W_Z0, PARK_WALK_GG_W_Z1, PARK_WALK_GG_W_X, PARK_WALK_GG_W_LEN,
  PARK_WALK_GG_E_X0, PARK_WALK_GG_E_X1, PARK_WALK_GG_E_Z, PARK_WALK_GG_E_W,
  PARK_WALK_GG_E_Z0, PARK_WALK_GG_E_Z1, PARK_WALK_GG_E_X, PARK_WALK_GG_E_LEN,
  PARK_PERGOLA_X, PARK_PERGOLA_Z,
  PARK_PERGOLA_EE_X, PARK_PERGOLA_EE_Z,
  PARK_PERGOLA_FF_X, PARK_PERGOLA_FF_Z,
  PARK_PERGOLA_GG_X, PARK_PERGOLA_GG_Z, PARK_PERGOLA_GG_OPEN_H, PARK_PERGOLA_GG_FLY,
  PARK_PERGOLA_GG_HALF_X, PARK_PERGOLA_GG_HALF_Z, PARK_PERGOLA_GG_POST_H,
  PARK_PERGOLA_GG_W, PARK_PERGOLA_GG_D, PARK_PERGOLA_GG_AABB,
  PARK_PERGOLA_GG_X0, PARK_PERGOLA_GG_X1, PARK_PERGOLA_GG_Z0, PARK_PERGOLA_GG_Z1,
  PARK_PERGOLA_GG_COLLIDER_PAD,
...
export function runMiamiParkKitGGTests() {
  fails.length = 0;
  passedCount = 0;

  const ctx = { blocked: () => false };
  const bench = gardenBenchGeom(PARK_BENCH_GG_X, PARK_BENCH_GG_Z);
  const benchW = gardenBenchGeom(PARK_BENCH_GG_W_X, PARK_BENCH_GG_W_Z);
  const benchE = gardenBenchGeom(PARK_BENCH_GG_E_X, PARK_BENCH_GG_E_Z);
  const parts = gardenBenchParts(PARK_BENCH_GG_X, PARK_BENCH_GG_Z);
  const voids = gardenBenchVoids(bench);
  const benchShapes = gardenBenchColliderShapes(bench);
  const gate = boardwalkGateGeom(PARK_PERGOLA_GG_X, PARK_PERGOLA_GG_Z);
  const sash = boardwalkGateVoid(gate, 'park-pergola-gg');
  const fly = FLY_VOIDS.find((v) => v.id === 'park-pergola-gg');
  const kit = flyColliderShapes();
  const west = gardenPathGeom(PARK_WALK_GG_W_X, PARK_WALK_GG_W_Z);
  const east = gardenPathGeom(PARK_WALK_GG_E_X, PARK_WALK_GG_E_Z);
  const spine = gardenPathGeom(PARK_WALK_GG_X, PARK_WALK_GG_Z);
  ...
  const parkG = pocketParkHull(POCKET_PARK_G_X, POCKET_PARK_G_Z);
  ...
  // ---- signed cells (Desi); 364 kit +17 m -------------------------------
  ok('G-park bench is signed 381 / 94.4',
    PARK_BENCH_GG_X === 381 && PARK_BENCH_GG_Z === 94.4
    && bench.x === 381 && bench.z === 94.4);
  ok('G-park bench is 364 kit +17 m',
    PARK_BENCH_GG_X === PARK_BENCH_FF_X + 17
    && PARK_BENCH_GG_Z === PARK_BENCH_FF_Z
    && PARK_BENCH_FF_X === 364);
  ok('0.8 m is center-to-spine of GG z0=95.2',
    PARK_WALK_GG_Z0 === 95.2
    && Math.abs(PARK_WALK_GG_Z0 - PARK_BENCH_GG_Z - 0.8) < 1e-9);
  ok('G-park bench yaw faces +Z toward z=96',
    PARK_BENCH_GG_YAW === 0 && PARK_BENCH_GG_YAW === PARK_BENCH_FF_YAW
    && bench.yaw === 0 && bench.yaw !== PARK_BENCH_YAW);
  ok('G-park bench is the same 1.8 m 3-seat kit',
    PARK_BENCH_GG_W === 1.8 && PARK_BENCH_GG_W === GARDEN_BENCH_W
    && PARK_BENCH_GG_W === PARK_BENCH_W && bench.w === GARDEN_BENCH_W);
  ok('G-park bench seat / depth / back / under-clear match the kit',
    PARK_BENCH_GG_SEAT_H === GARDEN_BENCH_SEAT_H
    && PARK_BENCH_GG_DEPTH === GARDEN_BENCH_DEPTH
    && PARK_BENCH_GG_BACK_H === GARDEN_BENCH_BACK_H
    && PARK_BENCH_GG_UNDER_CLEAR === GARDEN_BENCH_UNDER_CLEAR
    && PARK_BENCH_GG_X0 === 380.1 && PARK_BENCH_GG_X1 === 381.9);
  ok('G-park bench sits on the 381/96 hull',
    PARK_BENCH_GG_X0 > POCKET_PARK_G_X0 && PARK_BENCH_GG_X1 < POCKET_PARK_G_X1
    && PARK_BENCH_GG_Z0 > POCKET_PARK_G_Z0 && PARK_BENCH_GG_Z1 < POCKET_PARK_G_Z1
    && parkG.x === 381 && parkG.z === 96);

  ok('G-park pergola is signed 381 / 98.5',
    PARK_PERGOLA_GG_X === 381 && PARK_PERGOLA_GG_Z === 98.5
    && gate.x === 381 && gate.z === 98.5);
  ok('G-park pergola is 364 kit +17 m',
    PARK_PERGOLA_GG_X === PARK_PERGOLA_FF_X + 17
    && PARK_PERGOLA_GG_Z === PARK_PERGOLA_FF_Z);
  ok('G-park pergola opening is 2.20 m, fly +X',
    PARK_PERGOLA_GG_OPEN_H === 2.20
    && PARK_PERGOLA_GG_OPEN_H === GATE_POST_H
    && PARK_PERGOLA_GG_POST_H === GATE_POST_H
    && PARK_PERGOLA_GG_FLY === '+X' && gate.fly === '+X');
  ok('G-park pergola half-span 1.16 stays inside 100 / off spine z1=96.8',
    PARK_PERGOLA_GG_HALF_Z === GATE_HALF_Z && GATE_HALF_Z === 1.16
    && PARK_PERGOLA_GG_HALF_X === GATE_HALF_X
    && Math.abs(PARK_PERGOLA_GG_Z0 - 97.34) < 1e-9
    && Math.abs(PARK_PERGOLA_GG_Z1 - 99.66) < 1e-9
    && PARK_PERGOLA_GG_Z1 < POCKET_PARK_G_Z1
    && Math.abs(POCKET_PARK_G_Z1 - PARK_PERGOLA_GG_Z1 - 0.34) < 1e-9
    && Math.abs(PARK_PERGOLA_GG_Z0 - PARK_WALK_GG_Z1 - 0.54) < 1e-9
    && PARK_PERGOLA_GG_Z0 > PARK_WALK_GG_Z1);

  ok('west walk is signed 373→379.2 / z=98.5',
    PARK_WALK_GG_W_X0 === 373 && PARK_WALK_GG_W_X1 === 379.2
    && PARK_WALK_GG_W_Z === 98.5 && PARK_WALK_GG_W_X === 376.1
    && west.x0 === 373 && west.x1 === 379.2 && west.z === 98.5);
  ok('west walk is 1.8 m west of 381, 364 kit +17 m',
    Math.abs(381 - PARK_WALK_GG_W_X1 - 1.8) < 1e-9
    && PARK_WALK_GG_W_X0 === PARK_WALK_FF_W_X0 + 17
    && PARK_WALK_GG_W_X1 === PARK_WALK_FF_W_X1 + 17
    && PARK_WALK_GG_W_Z === PARK_WALK_FF_W_Z);
  ok('west walk width is 1.6 m, last slab in 379.2',
    PARK_WALK_GG_W_W === 1.6 && PARK_WALK_GG_W_W === GARDEN_PATH_W
    && PARK_WALK_GG_W_W === PARK_WALK_GG_W
    && PARK_WALK_GG_W_LEN === 6.2
    && PARK_WALK_GG_W_Z0 === 97.7 && PARK_WALK_GG_W_Z1 === 99.3
    && westSlabs.every((s) => s.x1 <= 379.2 + 1e-9)
    && lastWest && lastWest.x1 <= 379.2 + 1e-9);
  ok('west walk sash stays empty (stops short of posts)',
    PARK_WALK_GG_W_X1 === 379.2
    && PARK_WALK_GG_W_X1 < (381 - GATE_HALF_Z)
    && PARK_WALK_GG_W_X1 < PARK_PERGOLA_GG_X0
    && PARK_WALK_GG_W_X1 < sash.x0
    && Math.abs((381 - GATE_HALF_Z) - 379.84) < 1e-9);

  ok('east walk is signed 382.8→389 / z=98.5',
    PARK_WALK_GG_E_X0 === 382.8 && PARK_WALK_GG_E_X1 === 389
    && PARK_WALK_GG_E_Z === 98.5 && PARK_WALK_GG_E_X === 385.9
    && east.x0 === 382.8 && east.x1 === 389 && east.z === 98.5);
  ok('east walk is 1.8 m east of 381, 364 kit +17 m',
    Math.abs(PARK_WALK_GG_E_X0 - (381 + 1.8)) < 1e-9
    && PARK_WALK_GG_E_X0 === PARK_WALK_FF_E_X0 + 17
    && PARK_WALK_GG_E_X1 === PARK_WALK_FF_E_X1 + 17
    && PARK_WALK_GG_E_Z === PARK_WALK_FF_E_Z);
  ok('east walk width is 1.6 m, last slab ≤ 389',
    PARK_WALK_GG_E_W === 1.6 && PARK_WALK_GG_E_W === GARDEN_PATH_W
    && PARK_WALK_GG_E_LEN === 6.2
    && PARK_WALK_GG_E_Z0 === 97.7 && PARK_WALK_GG_E_Z1 === 99.3
    && eastSlabs.every((s) => s.x1 <= 389 + 1e-9)
    && lastEast && lastEast.x1 <= 389 + 1e-9
    && east.x1 === 389);
  ok('east walk sash stays empty (starts east of posts)',
    PARK_WALK_GG_E_X0 === 382.8
    && PARK_WALK_GG_E_X0 > PARK_PERGOLA_GG_X1
    && Math.abs(PARK_WALK_GG_E_X0 - PARK_PERGOLA_GG_X1 - 0.65) < 1e-9
    && PARK_WALK_GG_E_X0 > sash.x1
    && Math.abs((381 + GATE_HALF_X) - 382.15) < 1e-9);

  ok('west bench is signed 374.5 / 94.4 (−6.5 m mirror)',
    PARK_BENCH_GG_W_X === 374.5 && PARK_BENCH_GG_W_Z === 94.4
    && Math.abs(PARK_BENCH_GG_X - PARK_BENCH_GG_W_X - 6.5) < 1e-9
    && PARK_BENCH_GG_W_X === PARK_BENCH_FF_W_X + 17
    && benchW.x === 374.5 && benchW.z === 94.4
    && PARK_BENCH_GG_W_X0 === 373.6 && PARK_BENCH_GG_W_X1 === 375.4
    && PARK_BENCH_GG_W_YAW === 0);
  ok('east bench is signed 387.5 / 94.4 (+6.5), x1=388.4 stays in 389',
    PARK_BENCH_GG_E_X === 387.5 && PARK_BENCH_GG_E_Z === 94.4
    && Math.abs(PARK_BENCH_GG_E_X - PARK_BENCH_GG_X - 6.5) < 1e-9
    && Math.abs(PARK_BENCH_GG_E_X + PARK_BENCH_GG_W_X - 2 * PARK_BENCH_GG_X) < 1e-9
    && PARK_BENCH_GG_E_X === PARK_BENCH_FF_E_X + 17
    && benchE.x === 387.5 && benchE.z === 94.4
    && PARK_BENCH_GG_E_X0 === 386.6 && PARK_BENCH_GG_E_X1 === 388.4
    && PARK_BENCH_GG_E_X1 < 389 && POCKET_PARK_G_X1 === 389
    && PARK_BENCH_GG_E_YAW === 0);

  // ---- stay-puts --------------------------------------------------------
  ok('lots A–G stay 258 / 295 / 313 / 330 / 347 / 364 / 381 at z=84',
  ...
  ok('276 park / 347 park / F hull / G hull stay',
  ...
  ok('GG spine stays 373→389 / z=96',
    PARK_WALK_GG_X0 === 373 && PARK_WALK_GG_X1 === 389
    && PARK_WALK_GG_Z === 96 && PARK_WALK_GG_W === 1.6
    && PARK_WALK_GG_LEN === 16 && PARK_WALK_GG_X === 381
    && spine.x0 === 373 && spine.x1 === 389 && spine.z === 96);
  ...
  ok('does not merge with F-park 372',
    PARK_WALK_FF_X1 === 372 && PARK_WALK_GG_X0 === 373
    && POCKET_PARK_F_X1 === 372 && POCKET_PARK_G_X0 === 373
    && PARK_WALK_GG_W_X0 === 373 && PARK_WALK_GG_W_X0 > 372);

  // ---- leftover 8000–11000; 11k ceiling; no backfill --------------------
  ok('three walks eat ~45 m²',
    Math.abs(PARK_WALK_GG_LEN * PARK_WALK_GG_W
      + PARK_WALK_GG_W_LEN * PARK_WALK_GG_W_W
      + PARK_WALK_GG_E_LEN * PARK_WALK_GG_E_W - 45.44) < 1e-9);
  ok('G leftover after the walks is 8000–11000 and < 12800',
  ...
  ok('did not backfill past 11k',
    ...
    && fieldG.placed.length < plannedG
    && fieldG.cells.length === plannedG);
  ...
  ok('cells are reserved keepouts on leftover-city grade',
    inReserved(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)
    && inKeepout(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)
    && inReserved(PARK_PERGOLA_GG_X, PARK_PERGOLA_GG_Z)
    && inKeepout(PARK_PERGOLA_GG_X, PARK_PERGOLA_GG_Z)
    && inReserved(PARK_WALK_GG_W_X, PARK_WALK_GG_W_Z)
    && inKeepout(PARK_WALK_GG_W_X, PARK_WALK_GG_W_Z)
    && inReserved(PARK_WALK_GG_E_X, PARK_WALK_GG_E_Z)
    && inKeepout(PARK_WALK_GG_E_X, PARK_WALK_GG_E_Z)
    && inReserved(PARK_BENCH_GG_W_X, PARK_BENCH_GG_W_Z)
    && inReserved(PARK_BENCH_GG_E_X, PARK_BENCH_GG_E_Z)
    && groundHeight(PARK_BENCH_GG_X, PARK_BENCH_GG_Z) === CITY_Y
    && !onPavement(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)
    && !onBoardwalk(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)
    && !onRoadway(PARK_BENCH_GG_Z)
    && !onCrossStreet(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)
    && !onSidewalk(PARK_BENCH_GG_X, PARK_BENCH_GG_Z));
  ok('signed cells are not rejected',
    !gardenBenchRejected(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)
    && !gardenBenchRejected(PARK_BENCH_GG_W_X, PARK_BENCH_GG_W_Z)
    && !gardenBenchRejected(PARK_BENCH_GG_E_X, PARK_BENCH_GG_E_Z)
    && !boardwalkGateRejected(PARK_PERGOLA_GG_X, PARK_PERGOLA_GG_Z)
    && !gardenPathRejected(PARK_WALK_GG_W_X, PARK_WALK_GG_W_Z)
    && !gardenPathRejected(PARK_WALK_GG_E_X, PARK_WALK_GG_E_Z)
    && !gardenPathRejected(PARK_WALK_GG_X, PARK_WALK_GG_Z)
    && !pocketParkRejected(POCKET_PARK_G_X, POCKET_PARK_G_Z));
  ok('tryPlace drops the reserved kit',
    tryPlace(ctx, PARK_BENCH_GG_X, PARK_BENCH_GG_Z) === 0
    && tryPlace(ctx, PARK_PERGOLA_GG_X, PARK_PERGOLA_GG_Z) === 0
    && tryPlace(ctx, PARK_WALK_GG_W_X, PARK_WALK_GG_W_Z) === 0
    && tryPlace(ctx, PARK_WALK_GG_E_X, PARK_WALK_GG_E_Z) === 0
    && tryPlace(ctx, PARK_BENCH_GG_W_X, PARK_BENCH_GG_W_Z) === 0
    && tryPlace(ctx, PARK_BENCH_GG_E_X, PARK_BENCH_GG_E_Z) === 0);
  ok('misses leftoverLot G / F-park / helipad / 364',
    LEFTOVER_LOT_G_Z1 === 90 && PARK_BENCH_GG_Z0 > LEFTOVER_LOT_G_Z1
    && !leftoverLotOverlap(PARK_BENCH_GG_X, PARK_BENCH_GG_Z,
      PARK_BENCH_GG_W, PARK_BENCH_GG_DEPTH, 0.15)
    && !inLeftoverLotReserved(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)
    && !inHelipadReserved(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)
    && !inWarehouseReserved(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)
    && PARK_BENCH_GG_X0 > POCKET_PARK_F_X1 && POCKET_PARK_F_X1 === 372
    && PARK_WALK_GG_W_X0 > PARK_WALK_FF_X1);
  ok('benches do not kiss the GG spine or each other',
    PARK_BENCH_GG_Z1 < PARK_WALK_GG_Z0
    && PARK_BENCH_GG_W_X1 < PARK_BENCH_GG_X0
    && PARK_BENCH_GG_X1 < PARK_BENCH_GG_E_X0
    && !gardenPathSlabOverlap(PARK_BENCH_GG_X, PARK_BENCH_GG_Z,
      PARK_BENCH_GG_W, PARK_BENCH_GG_DEPTH, 0));
  ok('spine z1=96.8 is 0.9 m south of sash walks (grow-to-gap)',
    PARK_WALK_GG_Z1 === 96.8
    && PARK_WALK_GG_W_Z0 === 97.7 && PARK_WALK_GG_E_Z0 === 97.7
    && Math.abs(PARK_WALK_GG_W_Z0 - PARK_WALK_GG_Z1 - 0.9) < 1e-9
    && west.z0 > spine.z1 && east.z0 > spine.z1);

  // ---- sit-box and sash stay flyable voids ------------------------------
  ok('G-park sit-box is a void in front of the back (+Z)',
  ...
  for (const v of voids) {
    ...
    ok(`gg ${v.id} is flyable`, !hit, hit ? `blocked by ${hit.tag}` : '');
  }
  ok('G-park sit-box faces +Z toward z=96',
    sit && sit.z > bench.z && sit.y > CITY_Y + PARK_BENCH_GG_SEAT_H);
  ok('G-park bench colliders are legs + slats + back only',
  ...
  ok('park-pergola-gg fly void is published, sash empty',
    !!fly && fly.kind === 'kit' && fly.openH === 2.20
    && !!inFlyVoid(PARK_PERGOLA_GG_X, PARK_PERGOLA_GG_Z)
    && PARK_PERGOLA_GG_AABB === false);
  const sashHit = probeBlocked(kit, sash.x, sash.y, sash.z, 0.12);
  ok('G-park pergola sash is flyable', !sashHit,
  ...
  ok('walk colliders are per-slab, not a path AABB',
    ...
    && !westShapes.some((s) => Math.abs(s.sx - PARK_WALK_GG_W_LEN) < 0.2)
    && !eastShapes.some((s) => Math.abs(s.sx - PARK_WALK_GG_E_LEN) < 0.2));

  // ---- kiss = drop, never nudge -----------------------------------------
  ok('drop if the kit kisses leftoverLot G / F-park / helipad / 364',
    gardenBenchRejected(LEFTOVER_LOT_G_X, LEFTOVER_LOT_G_Z) === true
    && gardenPathRejected(LEFTOVER_LOT_G_X, LEFTOVER_LOT_G_Z) === true
    && boardwalkGateRejected(LEFTOVER_LOT_G_X, LEFTOVER_LOT_G_Z) === true
  ...
  ok('drop if sit-box / sash / spine is filled by a kiss',
    gardenBenchRejected(PARK_WALK_GG_X, PARK_WALK_GG_Z) === true
    && gardenPathRejected(PARK_PERGOLA_GG_X, PARK_PERGOLA_GG_Z) === true
    && gardenBenchRejected(PARK_BENCH_GG_X, PARK_BENCH_GG_Z) === false
    && gardenBenchRejected(380.1, 94.4) === true
    && gardenPathRejected(PARK_WALK_GG_W_X, PARK_WALK_GG_W_Z) === false
    && gardenPathRejected(PARK_WALK_GG_E_X, PARK_WALK_GG_E_Z) === false);
  ...
  ok('G-park kit reuses gardenBenchGeom / boardwalkGateGeom / gardenPathGeom',
    benchSrc.includes('gardenBenchGeom(PARK_BENCH_GG_X, PARK_BENCH_GG_Z)')
    && benchSrc.includes('gardenBenchGeom(PARK_BENCH_GG_W_X, PARK_BENCH_GG_W_Z)')
    && benchSrc.includes('gardenBenchGeom(PARK_BENCH_GG_E_X, PARK_BENCH_GG_E_Z)')
    && flySrc.includes('boardwalkGateGeom(PARK_PERGOLA_GG_X, PARK_PERGOLA_GG_Z)')
    && gardenSrc.includes('gardenPathGeom(PARK_WALK_GG_W_X, PARK_WALK_GG_W_Z)')
    && gardenSrc.includes('gardenPathGeom(PARK_WALK_GG_E_X, PARK_WALK_GG_E_Z)')
    && constants.includes('364 kit +17 m')
    && constants.includes('Do not merge F-park 372')
    && constants.includes('leftover ~8.2k')
    && constants.includes('11k is a ceiling')
    && constants.includes('No leftover lots on this merge')
    && constants.includes('PARK_BENCH_GG_X = 381')
    && constants.includes('PARK_PERGOLA_GG_X = 381')
    && constants.includes('PARK_WALK_GG_W_X0 = 373')
    && constants.includes('PARK_WALK_GG_E_X0 = 382.8')
    && constants.includes('PARK_BENCH_GG_W_X = 374.5')
    && constants.includes('PARK_BENCH_GG_E_X = 387.5')
    && constants.includes('POCKET_PARK_G_INSTANCES_MIN = 8000')
    && constants.includes('POCKET_PARK_G_INSTANCES_MAX = 11000')
  ...
  ok('no new geom family / leftover lots / foliage / 4.2 m slab',
    ...
    && !/export function parkPergolaGGeom/.test(constants)
    && !/export function gardenPathGGeom/.test(constants)
    && !/export function leftoverLotDirtGeom/.test(constants)
    && !/export function parkWalkGGWGeom/.test(constants)
    && !/export function parkWalkGGEGeom/.test(constants)
    && !/export function parkWalkGGGeom/.test(constants)
    && !/export function parkBenchGGGeom/.test(constants)
    && !existsSync(join(here, 'landmarks/gardenBenchG.js'))
    && !existsSync(join(here, 'landmarks/parkPergolaG.js'))
    && !existsSync(join(here, 'landmarks/gardenPathG.js'))
    && !existsSync(join(here, 'landmarks/parkWalkGGW.js'))
    && !leftover.includes('PARK_BENCH_GG_')
    && !leftover.includes('PARK_WALK_GG_W_')
    && !leftover.includes('PARK_PERGOLA_GG_')
    && !grass.includes('PARK_BENCH_GG_')
    && !park.includes('PARK_BENCH_GG_')
  ...
  ok('photo-mode / ACES / bloom / SSAO / colony HUD / Shackleton / Starship stay off this merge',
    ...
    && !constants.includes('Starship') && !preview.includes('PARK_BENCH_GG_')
    && !follow.includes('PARK_BENCH_GG_') && !checkpoints.includes('PARK_BENCH_GG_')
    && /const GRAVITY = 9\.81/.test(quad) && !quad.includes('PARK_BENCH_GG_')
    && planting.includes('export function tryPlace')
    && !planting.includes('PARK_BENCH_GG_')
    && blades.includes('placeBladePlan') && !blades.includes('PARK_BENCH_GG_'));
  ok('F leftover / E leftover / 276 leftover comments stay 8–11k',
  ...
  if (fails.length) {
    console.error('[miami-parkKitGG] FAIL');
    for (const f of fails) console.error('  -', f);
  } else {
    console.log('[miami-parkKitGG] ok', passedCount, 'checks',
      `lastEast.x1=${lastEast && lastEast.x1} placedG=${fieldG.placed.length}/${plannedG} placedF=${fieldF.placed.length} placedE=${fieldE.placed.length} placed276=${field276.placed.length}`);
  }
  return {
...
const isMain = typeof process !== 'undefined'
  && process.argv[1] && process.argv[1].endsWith('parkKitGGTest.js');
if (isMain) {
  const r = runMiamiParkKitGGTests();
